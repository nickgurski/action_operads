\documentclass{lms} % default font size is 10pt
\usepackage{latexsym}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{amsmath}
...
\extraline{This research was supported by EPSRC 134023.}
...
%
%   \theoremstyle{example}
   \newnumbered{nota}[equation]{Notation}
   \newnumbered{example}[equation]{Example}
   \newnumbered{Defi}[equation]{Definition}
   \newnumbered{rem}[equation]{Remark}
	\newnumbered{comment}[equation]{Comment}
	
	
   \title[Operads and tensor products]%
...
\classno{18D50, 18D10, 18D05, 16D90, 55N91}
...
Note that the operad of symmetric groups $\Sigma$ has its action operad structure determined by two auxilliary operations.  The first is the block sum of permutations which we denote by
\[
...
\[
\beta(k_{11}, \ldots, k_{1j_{1}}, k_{21}, \ldots, k_{2j_{2}}, \ldots, k_{nj_{n}}) = \beta\big( \beta(k_{11}, \ldots, k_{1j_{1}}), \ldots, \beta(k_{n1}, \ldots, k_{nj_{n}}) \big)
\]
holds.
...
\delta\big(\delta(f)\big) \delta\big(\beta(g_{1}, g_{m})\big)
...
\]
using the compatibility of $\beta$ and $\pi$ in axiom \eqref{eq1}.  By axiom \eqref{eq7}, this is composite of $\delta$'s  is then $\delta_{n; Q_{1}, \ldots, Q_{n}}$ where $Q_{i} = q_{i1} + \cdots + q_{im_{i}}$.  But by the definition of the $q_{ij}$, we immediately see that $Q_{i} = P_{i}$, so the $\delta(f)$ in \cref{eqn:opass} is equal to the $\delta(\delta(f))$ appearing in \cref{eqn:opass2}, concluding the proof of the operad associativity axiom.
...
\[
\mb{\Lambda}\mbox{-}Op(M) \rightarrow \mb{\Lambda}\mbox{-}Op(N)
\]
...
Any $\mb{\Lambda}$-operad in $\mb{Cat}$ gives rise to a 2-monad on $\mb{Cat}$ \cite{cg}.  In our case, that 2-monad (also denoted $E\Lambda$) is given by
\[
X \mapsto \coprod_{n \geq 0} E\Lambda(n) \times_{\Lambda(n)} X^{n}
\]
where the action of $\Lambda(n)$ on $E\Lambda(n)$ is given by the obvious multiplication action on the right, and the action of $\Lambda(n)$ on $X^{n}$ is given using $\pi_{n}:\Lambda(n) \rightarrow \Sigma_{n}$ together with the standard left action of $\Sigma_{n}$ on $X^{n}$ in any symmetric monoidal category.  It will be useful for our calculations later to give an explicit description of the categories $E\Lambda(n) \times_{\Lambda(n)} X^{n}$.  Objects are equivalence classes of tuples $(g; x_1, \ldots, x_n)$ where $g \in \Lambda(n)$ and the $x_{i}$ are objects of $X$, with the equivalence relation given by
\[
...
\begin{prop}
$\mb{\Lambda}_{\infty}$ operads in $\mb{Cat}$ are unique up to a zig-zag of pointwise equivalences of $\mb{\Lambda}$-operads.
\end{prop}
\begin{proof}
...
\begin{rem}
One should also note that combining Theorem \ref{preserveGop} with Corollary \ref{egisinf}, we get canonical $\mb{\Lambda}_{\infty}$ operads in the category of simplicial sets by taking the nerve (the nerve functor is represented by a cosimplicial category, namely $\Delta \subseteq \mb{Cat}$, so preserves products) and then in suitable categories of topological spaces by taking the geometric realization (once again, product-preserving with the correct category of spaces).  Thus we have something like a Barratt-Eccles $\mb{\Lambda}_{\infty}$ operad for any action operad $\mb{\Lambda}$.
\end{rem}

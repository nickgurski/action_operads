...
\item Order-reversing permutations/reflexive(?) groups: Here we would like to take $\Lambda(n) = C_2 = \{e, \tau_n\}$ with $\pi_n$ giving either the identity, or the order-reversing permutation on $n$ elements when applied to $\tau$. With these groups and the underlying permutations as described, we find that Axiom 1 is not satisfied:
  \[
    \pi_4(\beta(\tau_2,\tau)) = \tau = \trans{1}{4}\trans{2}{3} \neq \trans{1}{2}\trans{3}{4} = \beta(\trans{1}{2},\trans{1}{2}) = \beta(\pi_2(\tau_2),\pi_2(\tau_2)).
  \]
\item Hyperoctahedral groups: Can't remember what this fails to satisfy... Dan Graves came up with a counterexample, using the main action operad equivariance axiom, which was clearer than Zhang's description \cite{zhang-grp} - Example 2.28.

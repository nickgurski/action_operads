...
\item Hyperoctahedral groups: Can't remember what this fails to satisfy... Dan Graves came up with a counterexample, using the main action operad equivariance axiom, which was clearer than Zhang's description \cite{zhang-grp} - Example 2.28. Can show now that this is really saying that it fails Axiom \ref{eq8} of \ref{thm:charAOp} when taking $\delta_{1;n}(-1)$ to be the order-reversing permutation. Can also show that taking $\delta_{1;n}(-1)$ to be the negative identity matrix makes it fail on Axiom \ref{eq6} of \ref{thm:charAOp} instead - though it does then satisfy Axiom \ref{eq8}.

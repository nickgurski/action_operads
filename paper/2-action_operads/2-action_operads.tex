...
\item $\beta(\tau_1,\ldots,\tau_n), \delta(\sigma) \in \Sigma_{k_1 + \ldots + k_n}$ as described in \cref{conv1} \eqref{conv:beta_delta} and in \cref{exSigma} below.
\end{itemize}
...
\begin{conv}\label{conv:op_superscript}
...
\item Order-reversing permutations/reflexive(?) groups: Here we would like to take $\Lambda(n) = C_2 = \{e, t_n\}$ with $\pi_n$ giving either the identity, or the order-reversing permutation on $n$ elements when applied to $t_n$. With these groups and the underlying permutations as described, we find that Axiom \ref{eq1} of \cref{thm:charAOp} is not satisfied:
  \[
    \pi_4(\beta(t_2,t_2)) = t_4 = \trans{1}{4}\trans{2}{3} \neq \trans{1}{2}\trans{3}{4} = \beta(\trans{1}{2},\trans{1}{2}) = \beta(\pi_2(t_2),\pi_2(t_2)).
  \]
QQQ Needs clarifying.
\item Hyperoctahedral groups: Can't remember what this fails to satisfy... Dan Graves came up with a counterexample, using the main action operad equivariance axiom, which was clearer than Zhang's description \cite{zhang-grp} - Example 2.28. Can show now that this is really saying that it fails Axiom \ref{eq8} of \cref{thm:charAOp} when taking $\delta_{1;n}(-1)$ to be the order-reversing permutation. If considering elements of the hyperoctahedral groups to be signed permutation matrices, then can also show that taking $\delta_{1;n}(-1)$ to be the negative identity matrix makes it fail on Axiom \ref{eq6} of \cref{thm:charAOp} instead - though it does then satisfy Axiom \ref{eq8}.

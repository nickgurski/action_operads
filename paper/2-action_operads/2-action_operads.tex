...
\item The symbol $e$ will generically represent an identity element in a group. If we are considering a set of groups $\{ \Lambda(n) \}_{n \in \N}$ indexed by the natural numbers, then $e_{n}$ is the identity element in $\Lambda(n)$.
...
For an action operad $\ML$, we write $e_n$ for the identity element for the group $\Lambda(n)$. We drop the subscripts and just write $e$ when the index $n$ in $\Lambda(n)$ is either clear from context or unimportant to the argument at hand. Occasionally we will write $\Lambda_n$ in place of $\Lambda(n)$, especially in diagrams.
\end{nota}
...
\item Two less trivial examples are given by the braid groups, $\ML = \mb{B}$, and the ribbon braid groups, $\ML = \mb{RB}$. (A ribbon braid is given, geometrically, as a braid with strands replaced by ribbons in which we allow full twists. The actual definition of the ribbon braid groups is as the fundamental group of a configuration space in which points have labels in the circle, $S^{1}$; see \cite{sal-wahl}.)  In each case, the homomorphism $\pi$ is given by taking underlying permutations, and the operad structure is given geometrically by using the procedure explained after \cref{broperad}. We refer the reader to \cite{fie-br} for more information about braided operads, and to \cite{sal-wahl, wahl-thesis} for information about the ribbon case.
...
Every abelian group $A$ gives rise to action operad $A^{\bullet}$ as follows. The group $A^{\bullet}(n)$ is the direct sum of $n$ copies of $A$, $A^{n}$. The identity element is required to be $e \in A^{1}$, and the multiplication is defined by
  ...
  \]
where $\mb{b_{i}}$ is the string $b_{i1}, \ldots, b_{ik_{i}}$, and $a_{i} + \mb{b_{i}}$ is
  \[
...
A \textit{map of action operads} $f \colon  \ML \rightarrow \ML'$ consists of a map $f \colon \Lambda \rightarrow \Lambda'$ of the underlying operads such that
  \begin{enumerate}
...
Let $\ML$ be an action operad. The group operation of $\Lambda(0)$ coincides with the operation 
  \[
...
\begin{proof}
For a given action operad $\ML$, we put $U(\ML) = \left(\coprod_{\mathbb{N}} \Lambda(n), \coprod_{\mathbb{N}} \pi_n \right)$ and this easily extends to a mapping on morphisms using the universal property of the coproduct. The preservation of filtered colimits follows from the fact that these are computed pointwise, together with the fact that every map between action operads preserves underlying permutations.
...
Just as we had the definitions of operad, symmetric operad, and braided operad, we now come to the general definition of a $\ML$-operad, where $\ML$ is an action operad. Thereafter we begin to delve into the theory around $\ML$-operads. Beginning by recasting familiar operads as $\ML$-operads for particular choices of $\ML$, we then proceed to shadow standard operadic definitions for algebras and pseudoalgebras in this setting, characterising algebras using endomorphism $\ML$-operads, and deriving monads from $\ML$-operads. We show that there is an adjunction between $\ML\mbox{-}\mb{Op}$ and $\mb{\Sigma}\mbox{-}\mb{Op}$, and prove some technical results regarding maps between monads induced by $\ML$-operads.
...
\begin{Defi}\label{Defi:lamop}
  Let $\ML$ be an action operad. A \textit{$\ML$-operad} $P$ (in $\mb{Sets}$) consists of
    \begin{itemize}
...
such that the following two equivariance axioms hold.
  ...
  \[
    \mu^{P}(p \cdot g; y_{1}, \ldots, y_{n}) = \mu^{P}\left((p; y_{\pi(g)^{-1}(1)}, \ldots, y_{\pi(g)^{-1}(n)}\right) \cdot \mu^{\Lambda}(g; e_{k_1}, \ldots, e_{k_n}).
  \]
  \end{itemize}
\end{Defi}

\begin{Defi}
  The category $\ML\mbox{-}\mb{Op}$ of $\Lambda$-operads has objects the $\ML$-operads and morphisms those maps of operads which are levelwise equivariant with respect to the $\Lambda(n)$-actions.
\end{Defi}
...
\begin{example}
  \begin{enumerate}
    ...
    \item Let $\mb{\Sigma}$ denote the operad of symmetric groups with $\pi \colon \Sigma \rightarrow \Sigma$ the identity map. Then a $\mb{\Sigma}$-operad is a symmetric operad in the category of sets.
    \item Let $\mb{B}$ denote the operad of braid groups with $\pi_{n} \colon B_{n} \rightarrow \Sigma_{n}$ the canonical projection of a braid onto its underlying permutation. Then a $\mb{B}$-operad is a braided operad in the sense of Fiedorowicz \cite{fie-br}.
  \end{enumerate}
\end{example}
...
\begin{Defi}
  Let $P$ be a $\ML$-operad. An \textit{algebra} for $P$ consists of a set $X$ together with maps $\alpha_{n} \colon P(n) \times_{\Lambda(n)} X^{n} \rightarrow X$ such that the maps $\tilde{\alpha}_{n}$ satisfy the operad algebra axioms given in \cref{opalgax}.
\end{Defi}
...
\begin{Defi}
Let $P$ be a $\ML$-operad. The category of algebras for $P$, $P\mbox{-}\mb{Alg}$, has objects the $P$-algebras $(X, \alpha)$ and morphisms $f \colon  (X, \alpha) \rightarrow (Y, \beta)$ those functions $f \colon X \rightarrow Y$ such that the following diagram commutes for every $n$.
  \[
    \xy
...
Let $\ML$ be an action operad.
\begin{enumerate}
\item An adjunction exists between the category of $\ML$-operads and the category of symmetric operads, with right adjoint $\pi^{*} \colon \ML\mbox{-}\mb{Op} \rightarrow \mb{\Sigma}\mbox{-}\mb{Op}$ and left adjoint denoted $S$.
...
\item There exists a natural isomorphism of monads between $\underline{P}$ and $\underline{S(P)}$ for a $\ML$-operad $P$. In particular, these monads (and hence operads) have isomorphic categories of algebras.
\end{enumerate}
\end{thm}
\begin{proof}
...
Let $C,D$ be cocomplete symmetric monoidal categories. Let $\ML$ be an action operad, and $P$ be a $\ML$-operad in $C$. Let $F \colon C \rightarrow D$ be a symmetric lax monoidal functor. Then $FP$ is a $\ML$-operad in $D$, and there exists a monad map $(F,\psi) \colon (C,\underline{P}) \rightarrow (D, \underline{FP})$.
\end{prop}
\begin{proof}
...
\begin{Defi}
Let $\ML$ be an action operad, and let $X, Y$ be $\ML$-collections. We define the $\ML$-collection $X \circ Y$ to be
  \[
...
Let $\ML$ be an action operad.
  \begin{enumerate}
    ...
    \item The category $\mb{Mon}(\ML\mb{\mbox{-}Coll})$ of monoids in $\ML\mb{\mbox{-}Coll}$ is equivalent to the category of $\ML$-operads.
  \end{enumerate}
\end{thm}
...
Now that $\mathbb{\Lambda}$ has a monoidal structure, there is also a monoidal structure on the category of $\mathbb{\Lambda}$-collections
  \[
...
such that the following diagram commutes for every $g_{1} \in \Lambda(k_{1}), \ldots, g_{n} \in \Lambda(k_{n})$, where $\underline{k} = k_1 + \cdots + k_n$.
  ...
  \[
    \xy
      (0,0)*+{\prod_{i=1}^n Y(k_i) \times \mathbb{\Lambda}(k,\underline{k})}="a";
      (60,0)*+{\prod_{i=1}^n Y(k_i) \times \mathbb{\Lambda}(k,\underline{k})}="b";
      (60,-20)*+{Y^{\star n}(k)}="c";
      (0,-20)*+{\prod_{i=1}^n Y(k_i) \times \mathbb{\Lambda}(k,\underline{k})}="d";
      %
...
Finally we are in a position to prove \cref{operad=monoid}. We make heavy use of the following consequence of the Yoneda lemma: given any functor $F \colon \mathbb{\Lambda} \rightarrow \mb{Sets}$ and a fixed object $a \in \mathbb{\Lambda}$, we have a natural isomorphism
  \[
...
First we must show that $\ML\mbox{-}\mb{Coll}$ has a monoidal structure using $\circ$. To prove this, we must give the unit and associativity isomorphisms and then check the monoidal category axioms. First, note that the unit object is given as $I = \mathbb{\Lambda}(-,1)$. Then for the left unit isomorphism, we find that
  \begin{align*}

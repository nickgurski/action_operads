...
\begin{example}
QQQ Non-examples: Hyperoctahedral, and check the other skew-simplicial groups in \cite{Kra87}. Already know that Reflexive doesn't work with order-reversing perms.
\begin{enumerate}
\item Cyclic groups: In this case we would want to have $\Lambda(n) = C_n$, the cyclic group of order $n$, with $\pi_n$ being the underlying permutation. But these simply don't form an operad in the way we would like. E.g., by denoting the generator of $C_n$ by $c_n$:
  \begin{align*}
...
\item Order-reversing permutations/reflexive(?) groups: Here we would like to take $\Lambda(n) = C_2 = \{e, \tau_n\}$ with $\pi_n$ giving either the identity, or the order-reversing permutation on $n$ elements when applied to $\tau$. With these groups and the underlying permutations as described, we find that Axiom 1 of \ref{thm:charAOp} is not satisfied:
  \[
    \pi_4(\beta(\tau_2,\tau)) = \tau = \trans{1}{4}\trans{2}{3} \neq \trans{1}{2}\trans{3}{4} = \beta(\trans{1}{2},\trans{1}{2}) = \beta(\pi_2(\tau_2),\pi_2(\tau_2)).
  \]
\item Hyperoctahedral groups: Can't remember what this fails to satisfy... Dan Graves came up with a counterexample, using the main action operad equivariance axiom, which was clearer than Zhang's description \cite{zhang-grp} - Example 2.28. Can show now that this is really saying that it fails Axiom 8 of \ref{thm:charAOp} when taking $\delta_{1;n}(-1)$ to be the order-reversing permutation. Can also show that taking $\delta_{1;n}(-1)$ to be the negative identity matrix makes it fail on Axiom 6 of \ref{thm:charAOp} instead - though it does then satisfy Axiom 8.
\item Alternating groups: The image of the obvious map for $\delta$ does not lie in $A_n$: $\delta_{4;,0,0,1,1}(\trans{1}{2}\trans{3}{4})=\trans{1}{2} \notin A_2$.
\end{enumerate}
\end{example}
...
Suppose that $\pi_2 \colon \Lambda(2) \rightarrow \Sigma_2$ is surjective, then there exists $g \in \Lambda(2)$ such that $\pi_2(g) = \trans{1}{2}$. Considering a transposition $\trans{a}{a+1} \in \Sigma_n$, for $n > 2$, we see that
  ...
  \end{align*}
Since this is the image of an element in $\Lambda(n)$ and $\Sigma_n$ is generated by the adjacent transpositions, then $\pi_n$ is surjective. In this argument we have used Axiom 1 of \ref{thm:charAOp} and $\trans{1}{2}$ is in the $a^\text{\tiny th}$ position.
% where the underlined elements $\underline{e_1}$ consist of $n-a$ copies of $e_1$ to the left of $g$ and $n-a-1$ copies of $e_1$ to the right.
...
Now we will consider the case where $\pi_2$ is the zero map. Assume that $\pi_n$ is zero for some $n \geq 2$. Now let $\sigma \in \operatorname{Im} \pi_{n+1}$, so there exists $g \in \Lambda(n+1)$ such that $\pi_{n+1}(g) = \sigma$. Now $\delta_{n+1;\underline{1},0,\underline{1}}(\sigma)$ is an element of $\Sigma_n$ and
  \begin{align*}
    ...
    &= e_n,
  \end{align*}
using Axiom 4 of \ref{thm:charAOp}. Hence $\sigma = e_{n+1}$ or $\sigma = \trans{a}{a+1}$.

If $\sigma = e_{n+1}$ then $\pi_{n+1}$ is zero. But if $a \neq 1$, then using Axiom 4 again
  \begin{align*}
    ...
    &= e_n,
  \end{align*}
a contradiction. If $a = 1$, then $\trans{a}{a+1} = \trans{1}{2}$ and again using Axiom 4
  \begin{align*}

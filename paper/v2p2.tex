...
\begin{prop}\label{prop:star-of-circle}
  Let $X, Y$ be $\Lambda$-collections. Then there is an isomorphism
  \[
    \int^{m} X^{\star l}(m) \times Y^{\star m} \cong (X \circ Y)^{\star l}.
  \]
\end{prop}
\begin{proof}
We prove this by induction on $l$. 
Note that $X^{\star 1} \cong X$ by \cref{rem:yoneda-coend}, so the $l=1$ case reduces to constructing an isomorphism
\[
  \int^{m} X(m) \times Y^{\star m} \cong X \circ Y.
\]

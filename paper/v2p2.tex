...
\[
f_{!}P(n) \times f_{!}P(k_1) \times \cdots \times f_{!}P(k_n) \to f_{!}P\left( \sum k_i\right)
\]
...
& = \tau^{-1}(i)
\end{align*}
because the underlying permutation of $\sigma$ is equal to that of $f(\sigma)$. After substituting these simplifications into the above, we obtain \cref{eqn:lhs-fshriek}, completing the proof that $\tilde{m}$ coequalizes the two maps in \cref{eqn:coeq-m} and therefore induces a unique operadic multiplication on $f_{!}P$.

\ngnoteil{remaining to do: check unit, associativity, equivariance axioms; check functoriality (finishes 2)}
\ngnoteil{remaining to do:  (all of 3)}
%
%

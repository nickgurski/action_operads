...
\ngnoteil{merging part 2 into part 1, move this intro?}
...
\ngnoteil{maybe move this slightly for better visibility/numbering}
...
We will examine the adjunction from \cref{thm:pbaop} further in the case that the map $f$ is the underlying permutation map $\pi \colon \Lambda \to \Sigma$ for an action operad $(\Lambda, \pi)$. Using the formulas in the proof above, we examine the unit and counit separately.
...
\end{lem}
\begin{proof}
\ngnoteil{This is almost in May GILS, but doesn't have enough details. We should provide a few.}
\end{proof}
...
We now have the following proposition; its proof is standard\ngnote{But maybe there actually is no proof in GILS??} \cite{maygeom}, and we leave it to the reader.
...
We now check that the following diagram commutes, where underlined elements represent lists of the indicated elements indexed from $1$ to $n$, e.g.,
  ...
  \[
    ...
    \endxy
  \]
Square $\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {1}}}$ commutes by naturality of symmetries, square $\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {2}}}$ commutes by \cref{eq8} from \cref{thm:charAOp}, and square $\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {3}}}$ commutes by the definition of the coend. 
...
\end{align*}
by \cref{eq:fbullet} twice, the left action of $\Sigma_n$ on $n$-tuples as in \cref{rem:Sn-tuples}, and \cref{eq6} from \cref{thm:charAOp}.
\acnote{Did these use to come up as `Item'? Or have I broken something?}\ngnote{We can easily change them to whatever you like. They are just items in a list, not sure the best name for that}

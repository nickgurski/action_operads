...
To define $\delta \colon J_{n} \rightarrow J_{K}$ for natural numbers $n, k_{1}, \ldots, k_{n}$ and $K = \sum k_{i}$, let $t_{k} = s_{1,k} \in J_{k}$. Then we start by defining
  ...
  \]
%Note that, by Axiom \eqref{eq8} of \cref{thm:charAOp}, 
...
\begin{Defi}[(Pseudo-commutative structure for operads)]\label{def:ps-comm_operad}
...
\ngnoteil{From now: have we done as much of this as we want to do? I don't want to do any more.}
    \begin{itemize}

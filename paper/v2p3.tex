...
The limit of $F$, as a functor, is then also the 2-limit of $F$, as a 2-functor\ngnote{needs a ref, maybe just in Kelly's book somewhere?} by standard methods in enriched category theory.
In particular, every limit (or colimit) of such a diagram automatically inherits a 2-dimensional aspect to its universal property.
\end{rem}
...
\begin{itemize}
\item Here \emph{weak} means equational axioms are replaced by coherent isomorphisms. On its own, the reader might expect such a claim to mean that pseudo-$\EL$-algebras have an underlying monoidal, rather than strict monoidal, structure. This is not the case.

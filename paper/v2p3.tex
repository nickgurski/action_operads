...
The category $\mb{P}$ in Kelly's notation (defined in \cite[Section 2]{kelly_club0}) is the category $B\Sigma$ of \cref{Defi:BLambda}, or equivalently
 the result of applying $\underline{E\Sigma}$ to $1$ by \cref{prop:el1=bl}.
\end{rem}
...
\begin{itemize}
  ...
  \item natural transformations given by the set $\mathbf{g}$:  each element $x$ of $\mathbf{g}$ with $\pi(x) = \sigma_{x} \in \Sigma_{|x|}$ gives a natural transformation $\otimes_{|x|} \Rightarrow \otimes_{|x|} \circ \sigma$,
\end{itemize}
...
\begin{enumerate}
\item $\textrm{Comp}^0(X,f)$ is the collection $\{ e_0 \} \subseteq \SS$ consisting of the identity permutation in $\Sigma_0$; we denote the inclusion function above by $f_0$.
\item $\textrm{Comp}^1(X,f)$ is the collection $(X,f) \coprod \{\id\} \stackrel{f_1}{\to} \SS$, where $f_1$ agrees with $f$ on $X$ and $f_1(\id) = e_1 \in \Sigma_1$. 
\item Given $\textrm{Comp}^k(X,f)$ as a collection over $\SS$ via $f_k \colon \textrm{Comp}^k(X,f) \to \SS$, the elements of $\textrm{Comp}^{k+1}(X,f)$ are tuples $(x; y_1, \ldots, y_n)$ where
\begin{itemize}
\item $x \in \textrm{Comp}^1(X,f)$,
...
Let $\mathbb{S} = (\mathcal{G}, \mathcal{R})$ be a monoidal schema. An \emph{$\mathbb{S}$-monoidal category} consists of
\begin{itemize}
...
The above definition is meant to encapsulate the kinds of monoidal structures we regularly encounter.
\begin{enumerate}
\item Strict monoidal categories are the $\mathbb{T}$-monoidal categories for the monoidal schema $\mathbb{T}$ in which both $\mathcal{G}$ and $\mathcal{R}$ are empty.
\item Symmetric strict monoidal categories are the $\mathbb{P}$-monoidal categories for which
\begin{itemize}
\item $\mathcal{G} = \{\beta\}$ with $\pi(\beta) = (1 \ 2) \in \Sigma_2$ and
\item $\mathcal{R}$ consists of elements....
\end{itemize}
\item Braided strict monoidal categories are the $\mathbb{B}$-monoidal categories for which
\begin{itemize}
\item $\mathcal{G} = \{\beta\}$ with $\pi(\beta) = (1 \ 2) \in \Sigma_2$ and
\item $\mathcal{R}$ consists of elements....
\end{itemize}

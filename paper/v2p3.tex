...
%
%
%description of the set of objects of the coequalizer of $\mu$ and $\pi_2$ is still the set of orbits of $G$ acting on the objects of $C$ because the set of objects functor is a left adjoint. In particular, if $P$ is a $\Lambda$-operad and the action of $\Lambda(n)$ on $P(n)$ is not free, then the set of  objects of $\coeq{P}{A}{\Lambda}{n}$ is given by quotienting that set $\textrm{ob}P(n) \times \textrm{ob}A^n$ by the action of $\Lambda(n)$.
\end{rem}

\begin{Defi}[(2-limits)]\label{Defi:2limit}
\acnoteil{I put in the really spelled-out version, hopefully correctly}
\ngnoteil{I added a 2-d part to universal property 1, otherwise I think this looks good}
...
%To make this clearer, consider the object $[p;[q_1;x_{11}],[q_2;x_{21},x_{22}],[q_3;x_{31}]]$. The component of $\Phi$ at this object is given by the component of $\phi_{1,2,1}$ at the same object. 
...
We will eventually show in \cref{cor:pres2} that $\Lambda$-monoidal categories can be given in more familiar terms, as in Chapter 19 of \cite{yau_infinity_2021}.
...
An earlier version of this article constructed a pseudo-commutative structure for $EB$ as a $B$-operad, but contained an error and has been removed.
\end{rem}

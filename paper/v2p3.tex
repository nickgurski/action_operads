...
\ngmpar{not entirely sure what I want to say here, or if this is enough}
We note that if the action of $G$ on $C$ is not free, then the description of the set of objects of the coequalizer of $\mu$ and $\pi_2$ is still the set of orbits of $G$ acting on the objects of $C$ because the set of objects functor is a left adjoint. In particular, if $P$ is a $\Lambda$-operad and the action of $\Lambda(n)$ on $P(n)$ is not free, then the set of  objects of $\coeq{P}{A}{\Lambda}{n}$ is given by quotienting that set $\textrm{ob}P(n) \times \textrm{ob}A^n$ by the action of $\Lambda(n)$.
\end{rem}
...
The internal equivalences in $\wlmc$ are, by definition, those weak $\Lambda$-monoidal functors $F \colon M \to N$ for which there exists a weak $\Lambda$-monoidal functor $G \colon N \to M$ and invertible monoidal transformations $GF \cong 1_M$, $FG \cong 1_N$. By a generalization of the standard argument for plain monoidal functors\ngnote{can we put a ref here?}\acnote{I think this is one of those things that is not written down anywhere - best I can find is Remark 2.4.10 in "Tensor Categories", Etingof et al.},\acnote{Actually, is Prop 3.84 in Aguiar-Mahajan's book (Hopf Species, etc.) effectively doing this?}\acnote{Does it also sort of follow from Thm 5.1 in "Biequivs in Tricats"?}\acnote{Is it really all just doctrinal adjunction and mates somewhere? Theorem 1.5 in \cite{kelly-doc}?} a weak $\Lambda$-monoidal functor $F$ is an internal equivalence if and only if the underlying functor of $F$ is an equivalence of categories.
\end{rem}
...
\]
Define $[g]$ to be $[\tilde{g}; \underline{\id}]$. The three axioms in \cref{Defi:wk-lmc} follow immediately from the action operad axioms, completing the construction of the 2-functor $i$ on objects. Similar arguments apply to strict $\EL$-morphisms and $\EL$-transformations, and we leave them to the reader. The arguments above and \cref{prop:hom-set-lemma} show that the resulting 2-functor $i$ has image those weak $\Lambda$-monoidal category with strict underlying monoidal structure, finishing the proof of the first claim.

Now let $M$ be a weak $\Lambda$-monoidal category, and let $M_u$ denote its underlying monoidal category. By coherence for monoidal categories\ngnote{ref joyal-street, maybe my tricats book}, there is a strict monoidal category $\textrm{st} M_u$ and a monoidal equivalence $e \colon \textrm{st} M_u \to M_u$ given as follows.
\begin{itemize}
...
The first axiom is a consequence of \cref{eq6}, the second axiom is a consequence of \cref{eq9}, and the third axiom is a consequence of \cref{eq7}. We write $\textrm{st}^{\Lambda} M$ for this $\Lambda$-monoidal structure on $\textrm{st} M_u$. Since $\textrm{st} M_u$ is strict monoidal by construction and $e \colon \textrm{st} M_u \to M$ is known to be a monoidal equivalence, we can prove that $e$ is actually a $\Lambda$-monoidal equivalence $\textrm{st}^{\Lambda} M \to M$ by showing that $e$ is a $\Lambda$-monoidal functor. The single axiom in \cref{Defi:wk-lmf} requires the commutativity of the diagram below,
 \[
        ...
        \endxy
     \]
and that follows immediately from the third axiom in \cref{Defi:wk-lmc}. By \cref{rem:eq-in-wlmc}, this observation completes the proof that $e$ is an equivalence in the 2-category $\wlmc$.
\end{proof}

\begin{rem}[(Pseudo-$\EL$-algebras versus weak $\Lambda$-monoidal categories)]\label{rem:psELalg-vs-wlmc}
While satisfying in its brevity, one would expect \cref{cor:coherence-for-ulP} to be less useful in practice than the second part of \cref{thm:wlmc-to-lmc}, as that is the case for plain monoidal categories.
\end{rem}


\section{Group Actions and Cartesian $2$-monads}\label{sec:cart}

\ngnoteil{I slightly rewrote this. It is better but not amazing.}

This is the first of two sections to investigate the interaction between operads and pullbacks. The monads arising from a non-symmetric operad are always cartesian, as described in \cite[Appendix C]{leinster}. The monads that arise from symmetric operads, however, are not always cartesian and an example of where this fails is the symmetric operad for which the algebras are commutative monoids. In the case of $2$-monads we can consider the  strict $2$-limit analogous to the pullback, the $2$-pullback, and characterize when the induced $2$-monad from a $\Lambda$-operad is $2$-cartesian. We prove in \cref{cor:cart_cor,thm:cart_thm} that $P$ being 2-cartesian is equivalent to either free group actions, in the symmetric case, or a slight weakening of free group actions, in the general $\Lambda$-operad case.
...
\begin{rem}
As discussed in \cref{rem:lim-v-2lim}, the  $2$-pullback of a diagram is actually the same as the ordinary pullback in $\mb{Cat}$.\ngnote{kelly-elem citation used to be here, not sure if we need it}
\end{rem}
...
\begin{rem}[($\mb{P} = \underline{E\Sigma}(1)$)]
...
\begin{rem}[(Explicit description of clubs)]
...
\end{enumerate}
satisfying the axioms to be a monoid in the monoidal category of $E\Sigma$-collections. The objects of $K \circ K$  are tuples of objects of $K$, written $(x; y_{1}, \ldots, y_{n})$, where $\pi(x) = n$. In order to describe the morphisms of $K \circ K$, recall the description of the hom-sets of $E\Sigma(K)$ from \cref{prop:hom-set-lemma}.
A morphism
  \[
    (x; y_{1}, \ldots, y_{n}) \rightarrow (z; w_{1}, \ldots, w_{m})
  \]
exists only when $n=m$ (since $B\Sigma$ only has endomorphisms) and then consists of a morphism $f \colon x \rightarrow z$ in $K$ together with morphisms $g_{i} \colon y_{i} \rightarrow w_{f(i)}$ in $K$; here we have written $f(i)$ for the permutation $\pi(f)$ applied to the element $i$, following \cref{nota:perm_shorthand}.
\end{rem}
...
\ngnoteil{skipping the rest of this section for now}
...
We now turn to the operadic description of strict coboundary categories.
...
\end{lem}
\begin{proof}
This is obvious by \cref{cor:pres2}.
\end{proof}
...
\]
This equation is 3.9 in \cite{guillou_multiplicative}\ngnote{check this is the multiplicative equivariant one}.
\end{itemize}
\end{rem}
%   \[
%       \begin{bmatrix}
...
%\ngnoteil{start here, now I think this remark is wrong}
%
%\begin{remark}
%Remark 11.2 of \cite{guillou_multiplicative} describes the need for an extra equivariance axiom in the pseudo-commutative structure definition, some detail of which is also described in \cite{guillou_symmetric}. We also believed this to be true until a realisation that the equivariance axiom of \cite[Axiom (iii) of 11.1]{guillou_multiplicative} becomes the following requirement:
%          \[
...
\begin{proof}
We refer to the Axioms in \cref{def:ps-comm_operad} throughout. We begin the proof by defining an invertible modification $\gamma$ for the pseudo-commutativity for which the components are natural transformations $\gamma_{A,B}$. Let $[p;a_1,\ldots,a_n]$ be an object of $\coeq{P}{A}{\Lambda}{n}$ and $[q;b_1,\ldots,b_m]$ be an object of $\coeq{P}{B}{\Lambda}{m}$.
...
\begin{align*}
%[\lambda_{p,q} \cdot t_{m,n}^{-1}, 1] \colon 
...
For the unit axioms, we must compute the components of $\gamma$ precomposed with $\eta \times 1$ for the first axiom and $1 \times \eta$ for the second. Thus for the first unit axiom, we must compute the component of $\gamma$ at $\left( [\id;a], [q; b_{1}, \ldots, b_{m}] \right)$. By definition, this is the image of $(\lambda_{\id,q}\cdot t^{-1}_{m,1}, 1)$, and by Axiom \eqref{axiom:t_id} of \ref{def:ps-comm_operad} this is the identity. The second unit axiom follows similarly, using that $\lambda_{p, \id}$ and $t^{-1}_{1,n}$ are both the identity.
...
If $P$ is contractible and there exist $t_{m,n}$ as in \cref{def:ps-comm_operad}, then $P$ acquires a pseudo-commutative structure. Furthermore, it is symmetric if $t_{n,m} = t_{m,n}^{-1}$.
\end{cor}
\begin{proof}

...
To investigate operads in $\cat$ we will make use of $2$-monads and their algebras, specifically the notion of a pseudoalgebra for a $2$-monad. We assume familiarity with basic $2$-category theory\ngnote{cite johnson-yau book, kelly-street paper}, but cover the required definitions and theory related to $2$-monads here. For further reference, we refer the reader to \cite{BKP} and \cite{power-gen}.
...
\item morphisms $[f] \colon [c] \to [d]$, where $f \colon c \to d'$ and $d' \in [d]$.
\end{itemize}
\item The category $C/G$ is the coequalizer
    \[
        ...
        \endxy
    \]
where the top map is the action of $G$ on $C$, the bottom map is the projection onto $C$, and the coequalizing map $\varepsilon$ is defined by sending an object or morphism to its equivalence class in $C/G$.
\end{enumerate}
...
\]
 is free for every category $X$; this hypothesis is easily verified in the case that the action of $\Lambda(n)$ on $P(n)$ is itself free.

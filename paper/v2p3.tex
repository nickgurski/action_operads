...
A morphism
  ...
  \]
exists only when $n=m$ (since $B\Sigma$ only has endomorphisms) and then consists of a morphism $f \colon x \rightarrow z$ in $K$ together with morphisms $g_{i} \colon y_{i} \rightarrow z_{f(i)}$ in $K$; here we have written $f(i)$ for the permutation $\pi(f)$ applied to the element $i$.
\end{rem}

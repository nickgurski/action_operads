...
Now let $M$ be a weak $\Lambda$-monoidal category, and let $M_u$ denote its underlying monoidal category. By coherence for monoidal categories\ngnote{ref joyal-street, maybe my tricats book}, there is a strict monoidal category $\textrm{st}\ M_u$ and strict monoidal equivalence $e \colon \textrm{st}\ M_u \to M_u$ given as follows.
\begin{itemize}
...
Let $(n_i; x_{i1}, \ldots, x_{in_i})$ be objects of $\textrm{st}\ M_u$ for $i=1, \ldots, m$. For an element $g \in \Lambda(k)$, define $[g]$ to be the isomorphism $[\delta_{m; k_1, \ldots, k_m}(g)]$ from \cref{Defi:wk-lmc}.

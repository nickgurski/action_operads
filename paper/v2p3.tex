...
We note that if the action of $G$ on $C$ is not free, then the description of the set of objects of the coequalizer of $\mu$ and $\pi_2$ is still the set of orbits of $G$ acting on the objects of $C$ because the set of objects functor is a left adjoint.
\end{rem}
...
\item We call these transposition permutations as $\tau_{m,n}$ is the permutation given by taking the transpose of the $m \times n$ matrix with entries $(a_{i}, b_{j})$. 
\item The permutation $\tau_{m,n}$ has the effect of rearranging $m$ groups of $n$ things into $n$ groups of $m$ things.
\item \ngnoteil{include the tensor product commutativity relation from GMMO in its action operadic incarnation}
\end{itemize}
\end{rem}
%   \[
%       \begin{bmatrix}
...
so we define $(\gamma_{A,B})_{[p;a_1,\ldots,a_n],[q;b_1,\ldots,b_m]}$ to be the morphism which is the image of $(\lambda_{p,q}\cdot t_{m,n}^{-1}, 1)$ under the map
  ...
  \]
  \ngnoteil{start here}
  \ngnoteil{the point of the GMMO Axiom ii in 11.1 is to ensure that this is well-defined on equivalence classes of object}
  
  Do I need \cref{rem:obj-coeq}?
...
If $P$ is contractible and there exist $t_{m,n}$ as in \cref{def:ps-comm_operad}, then $\underline{P}$ acquires a pseudo-commutativity. Furthermore, it is symmetric if $t_{n,m} = t_{m,n}^{-1}$.
\end{cor}
\begin{proof}

...
In this chapter, we will explore the general definition of an operad, $P$, which is equipped with groups of equivariance, $\Lambda(n)$, which themselves bear the structure of an action operad (\ref{Defi:aop}). The group $\Lambda(n)$ will act on the right on the object $P(n)$, and the operad structure of $P$ will be required to respect this action. For certain choices of action operad, we will recover standard notions of operads such as symmetric operads, non-symmetric operads, and braided operads. The definitions here will, unless otherwise stated, apply in any symmetric monoidal category $\mathcal{V}$ in which the functors $X \otimes -, - \otimes X$ preserve colimits for every object $X \in \mathcal{V}$.
...
such that the following two equivariance axioms hold.
  ...
  \[
    \mu^{P}(p \cdot g; q_{1}, \ldots, q_{n}) = \mu^{P}\left(p; q_{g^{-1}(1)}, \ldots, q_{g^{-1}(n)}\right) \cdot \mu^{\Lambda}(g; e_{k_1}, \ldots, e_{k_n}).
  \]
  \end{itemize}
\end{Defi}

...
\begin{Defi} \label{coprodmapdef} Let $\tilde{\delta} := \mathrm{id}_{\ELnn}+\delta$ be the map defined from $\delta$ and the identity by using the universal property of the coproduct $\ELnnnn = \ELnn \coprod \ELnn$ in $E\Lambda\mbox{-}\mathrm{Alg}_S$. That is, $\tilde{\delta}$ is the map of $E\Lambda$-algebras which acts on generators by
\begin{eq*} \begin{array}{rlrlll}
...
Start checking here, Ed explained my mistake.

\begin{lem}\label{q_other_coeq} The functor $q$ (\cref{qdef}) is the coequalizer of $\tilde{\delta}$ and $\tilde{I}$ in $E\Lambda\mbox{-}\mathrm{Alg}_S$.
\end{lem}
\begin{proof}

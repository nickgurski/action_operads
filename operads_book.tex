...
\newcommand{\MorLn}{QQQ fix later}
\newcommand{\ELAlg}{QQQ fix later}
\newcommand{\LL}{\Lambda}
...
\end{prop}
\begin{proof} Since we already know that $\ELAlg$ is locally finitely presentable, the conditions for \cref{aftlfp} amount to showing that $(\_)_{\mathrm{inv}}$ preserves both limits and filtered colimits.
\begin{itemize}
...
\end{itemize}
Thus the underlying functor of $(\_)_{\mathrm{inv}}$ has a left adjoint.

To upgrade this into a 2-adjunction, we require MORE STUFF.
\end{proof}
...
\section{Splitting as a coherence theorem}

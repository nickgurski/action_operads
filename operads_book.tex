...
\begin{itemize}
\item $\Sigma_{n}$\nomenclature[N]{$\Sigma_{n}$}{symmetric group on $n$ letters} is the symmetric group on $n$ letters, and $B_{n}$\nomenclature[N]{$B_{n}$}{braid group on $n$ strands} is the braid group on $n$ strands.
...
The canonical example of an operad is the symmetric operad which we write as $\Sigma$\nomenclature[N]{$\Sigma$}{the symmetric operad}.  The set $\Sigma(n)$ is the set of elements of the symmetric group $\Sigma_{n}$, and the group action is just multiplication on the right.  The identity element $\textrm{id} \in \Sigma(1)$ is just the identity permutation on a one-element set.  Operadic composition in $\Sigma$ will then be given by a function
\[
...
\begin{example}
One can form an operad $Br$ where $Br(n)$ is the underlying set of the $n$th braid group, $B_{n}$.  This is done in much the same way as we did for the symmetric operad, and the collection of maps $\pi_{n}:B_{n} \rightarrow \Sigma_{n}$ giving the underlying permutations constitutes an operad map (of non-symmetric or braided operads) operads $Br \rightarrow \Sigma$.
\end{example}
...
\item Two less trivial examples are given by the braid groups, $\Lambda = \mb{B}$\nomenclature[N]{$\mb{B}$}{action operad of braid groups}, and the ribbon braid groups, $\Lambda = \mb{RB}$\nomenclature[N]{$\mb{RB}$}{action operad of ribbon braid groups}.  (A ribbon braid is given, geometrically, as a braid with strands replaced by ribbons in which we allow full twists.  The actual definition of the ribbon braid groups is as the fundamental group of a configuration space in which points have labels in the circle, $S^{1}$; see \cite{sal-wahl}.)  In each case, the homomorphism $\pi$ is given by taking underlying permutations, and the operad structure is given geometrically by using the procedure explained after Definition \ref{broperad}.  We refer the reader to \cite{fie-br} for more information about braided operads, and to \cite{sal-wahl, wahl-thesis} for information about the ribbon case.
...
\begin{Defi} For each $n \in \mathbb{N}$\nomenclature[N]{$\mathbb{N}$}{the set of natural numbers}, the \emph{ribbon braid group} $RB_n$\nomenclature[N]{$RB_{n}$}{ribbon braid group on $n$ ribbons} is the group whose presentation is the same as that of the braid group $B_n$, except with the addition of $n$ new generators $t_1, ..., t_n$, known as the \emph{twists}. These twists all commute with one other, and also commute with all braids except in the following cases:
...
\begin{example}
Every abelian group $A$ gives rise to action operad $A^{\bullet}$\nomenclature[N]{$A^{\bullet}$}{action operad of an abelian group $A$} as follows.  The group $A^{\bullet}(n)$ is the direct sum of $n$ copies of $A$, $A^{n}$.  The identity element is required to be $e \in A^{1}$, and the multiplication is defined by
\[
...
The category $\mb{AOp}$ of action operads has objects which are action operads $\mb{\Lambda}$ and
...
\begin{example}
One can easily form new action operads from old ones by taking limits.  To take a limit of a diagram in $\mb{AOp}$, one forgets down to the category of operads over $\Sigma$ and takes the limit there.  Concretely, products in $\mb{AOp}$ are computed as products in $\mb{Op}/\Sigma$\nomenclature[C]{$C/c$}{the slice category of $C$ over an object $c$ in $C$} (see QQQ) 
...
\begin{Defi}
The category $\Lambda\mbox{-}\mb{Op}$\nomenclature[C]{$\Lambda\mbox{-}\mb{Op}$}{of $\Lambda$-operads and maps of operads which are levelwise equivariant with respect to the $\Lambda(n)$-actions} of $\Lambda$-operads has objects the $\Lambda$-operads and morphisms those maps of operads which are levelwise equivariant with respect to the $\Lambda(n)$-actions.
\end{Defi}
...
\item Let $\mb{Br}$ denote the operad of braid groups with $\pi_{n}:B_{n} \rightarrow \Sigma_{n}$ the canonical projection of a braid onto its underlying permutation.  Then a $\mb{Br}$-operad is a braided operad in the sense of Fiedorowicz \cite{fie-br}.
\end{enumerate}
\end{example}
...
Let $X$ be a set.  Then the endomorphism operad of $X$, denoted $\mathcal{E}_{X}$\nomenclature[N]{$\mathcal{E}_{X}$}{the endomorphism operad of a set $X$}, is given by the sets $\mathcal{E}_{X}(n) = \mb{Sets}(X^{n}, X)$ with the identity function in $\mathcal{E}_{X}(1)$ giving the unit element and composition of functions giving the composition operation.  Concretely, composition is given by the formula
\[
...
\begin{Defi}
Let $P$ be a $\Lambda$-operad.  Then $P$ induces an endofunctor of $\mb{Sets}$, denoted $\underline{P}$\nomenclature[N]{$\underline{P}$}{the monad induced by an operad $P$}, by the following formula.
 \[
...
\item The category $\mb{Mon}(\Lambda\mb{\mbox{-}Coll})$\nomenclature[C]{$\mb{Mon}(C)$}{of monoids in $C$ and morphisms preserving the monoid structure} of monoids in $\Lambda\mb{\mbox{-}Coll}$ is equivalent to the category of $\Lambda$-operads.
\end{enumerate}
\end{thm}
...
\begin{itemize}
\item The $2$-category $P\mbox{-}\mb{Alg}_{s}$\nomenclature[C]{$P\mbox{-}\mb{Alg}_{s}$}{the $2$-category of strict $P$-algebras, strict morphisms, and $P$-transformations for an operad (or ($2$-monad) $P$} consists of strict $P$-algebras, strict morphisms, and $P$-transformations.
\item The $2$-category $\mb{Ps}\mbox{-}P\mbox{-}\mb{Alg}$\nomenclature[C]{$\mb{Ps}\mbox{-}P\mbox{-}\mb{Alg}$}{the $2$-category of strict $P$-pseudoalgebras, pseudomorphisms, and $P$-transformations for an operad (or $2$-)monad) $P$} consists of $P$-pseudoalgebras, pseudomorphisms, and $P$-transformations.
\end{itemize}
\end{Defi}
...
\begin{enumerate}
\item Let $X$ be a set.  We define the \textit{translation category} $EX$\nomenclature[C]{$EX$}{translation category of a set $X$} to have objects the elements of $X$ and morphisms consisting of a unique isomorphism between any two objects.
\item Let $G$ be a group.  The category $BG$\nomenclature[C]{$BG$}{delooping of a group $G$ as a category} has a single object $*$, and hom-set $BG(*,*) = G$ with composition and identity given by multiplication and the unit element in the group, respectively.
\end{enumerate}
\end{Defi}
...
\begin{thm}\label{preserveGop}Hi John,
...
Let $J_{n}$\nomenclature[N]{$J_n$}{the $n$-fruit cactus group} be the group generated by symbols $s_{p,q}$ for $1 \leq p < q \leq n$ subject to the following relations.
\begin{enumerate}
...
QQQ For notation, is $\moncat$ the $2$-category or category?
In this section, we will compare the coequalizer of a diagram of $\ML$-monoidal categories as it is computed in $\ML\mbox{-}\moncat$\nomenclature[C]{$\ML\mbox{-}\moncat$}{of $\ML$-monoidal categories and $\ML$-monoidal functors}, in $\moncat$\nomenclature[C]{$\moncat$}{of monoidal categories and monoidal functors}, and in $\cat$. Our goal will be to show that the forgetful functors 
\[
...
\begin{Defi} Given an $E\Lambda$-algebra $X$, we'll denote by $X_{\mathrm{inv}}$\nomenclature[N]{$X_{\mathrm{inv}}$}{the sub-$E\Lambda$-algebra of $X$ of objects invertible under tensor product} the sub-$E\Lambda$-algebra of $X$ containing all objects which are invertible under tensor product, and all of the isomorphisms between them. \end{Defi} 
...
\begin{Defi}
Let $L_n = L\Big(E\Lambda(\underline{n}) \Big)$\nomenclature[C]{$L_n$}{the free $\ML$-monoidal category on $n$ invertible objects}, where $\underline{n} = \{x_1, \ldots, x_n \}$ is a set with $n$ elements.
\end{Defi}
...
\begin{eq*} E\Lambda\mbox{-}\mb{Alg}_S(L_n, X) \quad \cong \quad (X_{\mathrm{inv}})^n, \end{eq*}
2-natural in $X$.
\end{thm}
\begin{proof}
...
\begin{eq*}\begin{array}{rll}
		...
		& \cong & \mb{Cat}( \underline{n}, X_{inv}) \\
		& \cong & X_{inv}^n
\end{array}
 \end{eq*}
...
\begin{cor} \label{epi} The component of the unit of the adjunction $L \dashv (-)_{inv}$ at $E\Lambda(\underline{n})$,  $\eta_{E\Lambda(\underline{n})}: E\Lambda(\underline{n}) \to (L_n)_{inv}$, is an epimorphism in $E\Lambda\mbox{-}\mb{Alg}_S$.
\end{cor}
\begin{proof}
...
%\begin{eq*} E\Lambda\mbox{-}\mb{Alg}_S( \, \ELn, \, \mathrm{E}H \, ) \quad \cong \quad \mb{Mon}( \, \mathrm{Ob}(\ELn), \, H \, ) \end{eq*}
...
%\begin{Defi} For a given action operad $G$, denote by $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$ the full subcategory of $\mb{Mon}$ on those monoids whose multiplication is invariant under the permutations in $\mathrm{im}(\pi)$. That is, a monoid $M$ is in $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$ if and only if
%\begin{eq*} m_1, ..., m_n \in M, \, g \in G(n) \quad \implies \quad m_1 \cdot ... \cdot m_n \, = \, m_{\pi(g)^{-1}(1)} ... m_{\pi(g)^{-1}(n)} \end{eq*}
%\end{Defi}
%
%Of course, by \cref{surjortriv} there are really only two examples of such an $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$. If the underlying permutations of $G$ are trivial, then $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$ is just the whole of the category $\mb{Mon}$; if  instead $G$ is crossed then we are asking for monoids whose multiplication is invariant under arbitrary permutations from $\mathrm{S}$, and so $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$ is just the category of \emph{commutative} monoids, $\cmon$. Regardless, when we are working with an arbitrary action operad $G$, the category $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$ is exactly the collection of possible connected components that we were looking for.
%
...
%\begin{eq*} \pi_0: E\Lambda\mbox{-}\mb{Alg}_S \to \mathrm{im}(\pi)\mbox{-}\mb{Mon} \end{eq*}
...
%
%There exists an inclusion of 2-categories $\mathrm{D}: \mb{Sets} \hookrightarrow \mb{Cat}$ which allows us to view any set $S$ as a \emph{discrete category}, one whose objects are just the elements of $S$ and whose morphisms are all identities. If the given set also happens to be a monoid $M$, then there is an obvious way to see the discrete category $\mathrm{D}M$ as a monoidal category, and so we have a similar inclusion $\mb{Mon} \hookrightarrow \mathrm{MonCat}$. Moreover, for any action operad $G$ and object $M$ of the category $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$, there is a unique way to assign an $E\Lambda$-action to the discrete category $\mathrm{D}M$. This works because for any elements $m_1, ..., m_n \in M$ and $g \in G(n)$, the morphism $\alpha(g; \mathrm{id}_{m_1}, ..., \mathrm{id}_{m_n})$ must have source and target 
...
%
%\begin{Defi} The functor $\mathrm{D}: \mb{Sets} \hookrightarrow \mb{Cat}$ extends naturally to a functor $\mathrm{im}(\pi)\mbox{-}\mb{Mon} \to E\Lambda\mbox{-}\mb{Alg}_S$, which we will also call $\mathrm{D}$.
%\end{Defi}
%
%\begin{prop}\label{concompadj} $\mathrm{D}: \mb{Sets} \hookrightarrow \mb{Cat}$ is a right adjoint to the functor $\pi_0: E\Lambda\mbox{-}\mb{Alg}_S \to \mathrm{im}(\pi)\mbox{-}\mb{Mon}$. 
%\end{prop}
%\begin{proof}
...
%\begin{eq*} E\Lambda\mbox{-}\mb{Alg}_S( \, X, \mathrm{D}M \, ) \quad \cong \quad \mathrm{im}(\pi)\mbox{-}\mb{Mon}( \, \pi_0(X), M \, ) \end{eq*}
%This isomorphism is natural in both coordinates, since for any $G: X \to X'$ in $E\Lambda\mbox{-}\mb{Alg}_S$ and $h : M \to M'$ in $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$, 
...
%& & \\
%E\Lambda\mbox{-}\mathrm{Alg}_S(X', \mathrm{D}M') \ar[rr, "\sim"] & & \mathrm{im}(\pi)\mbox{-}\mb{Mon}\big( \, \pi_0(X'), M' \, \big) 
%\end{tikzcd} \end{eq*}
...
%\end{prop}
%\begin{proof}
%Let $H$ be a group which is also an object of $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$, and let $h: \pi_0(\ELn) \to H$ be a monoid homomorphism. By \cref{concompadj} there is a homset isomorphism
%\begin{eq*} E\Lambda\mbox{-}\mb{Alg}_S( \, \ELn, \, \mathrm{D}H \, ) \quad \cong \quad \mathrm{im}(\pi)\mbox{-}\mb{Mon}( \, \pi_0(\ELn), \, H \, ) \end{eq*}
...
%\end{tikzcd} \end{eq*}
%Applying the functor $\pi_0$ everywhere, we see that $\pi_0(u)$ must also factor $h$ through the homomorphism $\pi_0(\eta)$. Moreover, since $\eta$ is an epimorphism and $\pi_0$ a left adjoint functor, $\pi_0(\eta)$ is an epimorphism too, and so $\pi_0(u)$ is the only map with this property. Therefore, any monoid homomorphism $\pi_0(\ELn) \to H$ will factor uniquely through $\pi_0(L_n)$, so long as $H$ is in $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$.  
%
%Now consider another monoid homomorphism $k: \pi_0(\ELn) \to K$, where this time $K$ is still a group but not necessarily in $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$. From \cref{pi0}, we know that $\pi_0(\ELn)$ is still an object of $\mathrm{im}(\pi)\mbox{-}\mb{Mon}$, and from this we can conclude that the image $\mathrm{im}(k)$ will be too:
...
%However, we are trying to extract information about the morphisms of $L_n$ by building some sort of left adjoint functor. Presumably we will also be able to apply it to other $E\Lambda$-algebras, some of which won't have all of their objects invertible, and so we can't just use $\mathrm{Mor}(-): E\Lambda\mbox{-}\mb{Alg}_S \to \mb{Mon}$. What we need is a way to modify the morphism monoid of a category so that both composition and tensor product are recoverable from a single operation. Of course, there is one very easy method for achieving this --- simply force $\otimes$ and $\circ$ to be equal.

\begin{Defi} Let $\mathrm{M} : \moncat \to \mb{Mon}$ be the functor which sends a monoidal category $X$ to the quotient of its monoid of morphisms by the relation that sets $\otimes = \circ$. :
...
% whatever method we would have used to recover $\mathrm{Mor}(L_n)$ from $\mathrm{M}(L_n)^{\mathrm{ab}}$ will still let us recover $\mathrm{Mor}(L_n) = \mathrm{Mor}(L_n)^{\mathrm{gp}}$ from $\mathrm{M}(L_n)^{\mathrm{gp},\mathrm{ab}}$.
 ...
 We will eventually prove that the morphisms of $L_n$ actually form a group under tensor product \cref{??}, so instead of working directly with the functor $\mathrm{M}(\, - \,)^{\mathrm{ab}}: \moncat \to \cmon$, we will focus on its composite with the group completion functor, $gp : \cmon \to \mb{Ab}$. We end with a brief exploration of this new functor $\mathrm{M}(\, - \,)^{\mathrm{gp},\mathrm{ab}}$. 
...
%\begin{eq*} \begin{tikzcd}
%\mb{Mon} \ar[rr, "(\, \_ \,)^{\mathrm{gp}}"] \ar[d, "(\, \_ \,)^{\mathrm{ab}}"'] & & \mb{Grp} \ar[d, "(\, \_ \,)^{\mathrm{ab}}"] \\
...
\begin{prop}\label{coeqsurj} Let $\phi, \phi' : X \to Y$ be a pair of $E\Lambda$-algebra maps, and $k: Y \to Z$ their coequalizer in $E\Lambda\mbox{-}\mb{Alg}_S$. If the monoid $\mathrm{Ob}(Z)$ is also a group, then the functor $k$ is surjective.
\end{prop}
\begin{proof}
Since the functor $\mathrm{Ob} : E\Lambda\mbox{-}\mb{Alg}_S \to \mb{Mon}$ is a left adjoint, it preserves all colimits so the monoid homomorphism $\mathrm{Ob}(k): \mathrm{Ob}(Y) \to \mathrm{Ob}(Z)$ is the coequalizer of the parallel pair $\mathrm{Ob}(\phi), \mathrm{Ob}(\phi')$ in the category of monoids. Every coequalizer is a regular epimorphism, and in the category of monoids regular epimorphisms coincide with  surjective functions, so $k$ is surjective on objects.
...
%
%\begin{Defi}\label{Cdef} Let the strict monoidal functor $c: \ELnn \to C$  be the cokernel of the underlying monoidal functor of $\delta$ in $\mathrm{MonCat}_S$. %This map definitely exists because $\mathrm{MonCat}$ is cocomplete, and like with $q$ we can show that its target has a group of objects.
%\end{Defi}
%
...
\end{prop}
\begin{proof}
From \cref{Moradj}, we know that $\mathrm{M}(\, \_ \,)^{\mathrm{gp, ab}}: \moncat \to \mb{Ab}$ is a left adjoint functor. This means that it preserves all colimits in $\moncat$, including the coequalizer use to define $\tilde{c}$, which from \cref{c'=q} we now know is really $q$.  Thus

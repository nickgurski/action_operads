% \immediate{makeindex -s operads_book.ist -o operads_book.gls operads_book.glo}
...
\renewcommand\nomgroup[1]{%
  \item[\bfseries
  \ifstrequal{#1}{C}{Catalogue of Categories}{%
  \ifstrequal{#1}{N}{Glossary of Notation}{}}%
]}
\usepackage[all]{xy}
\usepackage{tikz}
...
\begin{rem}
I think the following is true, although don't have time to go through it right now.
\begin{enumerate}
\item Look at Lack's \emph{Codescent and coherence} paper, section 4.1. This outlines how the free algebra functor for any braided operad preserves whatever colimits are preserved by $A \mapsto A^n$ for finite $n$. I think any $\ML$ will work here.
\item These functors all preserve reflexive coequalizers. I want to actually find this explicitly written down though.
\item Any monadic functor creates colimits that it preserves, so both $\ML\moncat \to \cat$ and $\moncat \to \cat$ create reflexive coequalizers, so $\ML\moncat \to \moncat$ does too.
\item I think this does everything through 4.4.1 (p102).
...
% \nomenclature[C]{$E(X^2)$}{expected value of $X^2$} \nomenclature[N]{$Br_{n}$}{braid group on $n$ strands}
% \index{group}

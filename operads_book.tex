...
\begin{proof}
  Note first that
    ...
    \end{align*}
  so for the first claim that $\mu(e_2; g, h) = g \cdot h$ it will suffice to show that $\mu(e_2; g, e_0) = g$ and $\mu(e_2; e_0, h) = h$ for all $g, h \in \Lambda(0)$. We will use the fact that $\mu(e_0; ) = e_0$, which follows below from a similar argument found in the second part of Lemma \ref{calclem}.
    \begin{align*}
      \mu(e_0;)\mu(e_0;) &= \mu\left(e_0^2;\right) \\
      &= \mu(e_0;).
    \end{align*}
  Since $\mu(e_0;)$ is an idempotent element of $\Lambda(0)$, then it must be the identity $e_0$. Now we have this, the following sequence of calculations shows that $\mu(e_2; g, e_0) = g$, while a similar calculation would show that $\mu(e_2; e_0, h) = h$. Using Lemma \ref{calclem},
    \begin{align*}
      g &= \mu(e_1; g) \\
      &= \mu(\mu(e_2; e_0, e_1); g) \\
...
\begin{rem}
  The requirement in Definition \ref{def:ps-alg} of a natural isomorphism $\varphi_\eta$ is to induce a natural isomorphism $\tilde{\varphi}_\eta$. This requirement is really of a natural isomorphism
    \[
      \xy
...
%     I = \alpha_0(e_0;).
%   \]
% Strict associativity follows from the the $\EL$-algebra axioms and Lemma \ref{calclem} as follows.
%   \begin{align*}
...
% Next we specify the permutation isomorphisms. Given $\sigma \in \Lambda(n)$, there is a unique isomorphism
%   \[
...
\begin{rem}
  Most often we will consider action morphisms as in Definition \ref{def:actionmorphism} where each of the $f_i$ is an identity, in which case we will use a shorthand notation
    \[
...
\begin{thm} The algebra $L_n$ is the free $\ML$-monoidal category on $n$ invertible objects:  for any other $\ML$-monoidal category $X$, we have an isomorphism of categories
  \[

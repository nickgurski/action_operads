...
\begin{prop} \label{stGnsub} $(s \times t)(\ELn)$ is (isomorphic to) a submonoid of $\mathrm{Mor}(\ELn)$, and $\mathrm{Mor}(\ELn) \to (s \times t)(\ELn)$ is a split epi of monoids.
\end{prop}
\begin{proof}
...
which agrees on the generators in the source. By construction, we have
...
\begin{rem}\label{choose_rho_later}
Note that, at this point, $\rho$ can be any function satisfying the criteria in \cref{rho_ww'}. Later we will make a specific choice.
\end{rem}

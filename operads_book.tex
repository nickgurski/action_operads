...
\end{lem}
\begin{proof}
QQQ Assume that we've shown that $gh = \mu(e_2;g,h)$.

Then
  \begin{align*}
    \mu(e_2;g,h) &= \mu(e_2 \cdot e_2;e_0 \cdot g, h \cdot e_0) \\
...
and
  ...
  \end{align*}
Hence the operation is commutative and so $\Lambda(0)$ is abelian.
\end{proof}
...
Let $C,D$ be cocomplete symmetric monoidal categories.  Let $\ML$ be an action operad, and $P$ be a $\ML$-operad in $C$. Let $F \colon C \to D$ be a symmetric lax monoidal functor. Then $FP$ is a $\ML$-operad in $D$, and there is a monad map $(F,\psi) \colon (C,\underline{P}) \to (D, \underline{FP})$.
\end{prop}
\begin{proof}
Theorem \ref{preserveGop} describes how the functor $F$ can be used to describe a functor
  \[
    \mb{\Lambda}\mbox{-}Op(C) \rightarrow \mb{\Lambda}\mbox{-}Op(),
  \]
from which we see that $FP$ is a $\ML$-operad in $D$.

Now the functor $F$ constitutes the $1$-cell of the monad map, while $\psi$ is required to be a natural transformation as below.
  \[
    \xy
      (0,0)*+{C}="a";
      (20,0)*+{D}="b";
...
The first morphism is a composite of the coherence cells of the type
  ...
  \]
for the symmetric lax monoidal functor $F$, while the second morphism is the induced morphism out of the coproduct. Naturality follows from that of the component morphisms. It is then straightforward to see that the monad morphism diagrams commute since diagrams involved consist of instances of the coherence axioms for $F$ along with naturality of the coherence cells.
\end{proof}

\begin{prop}\label{opmap_to_monadmap}
Let $C$ be a cocomplete symmetric monoidal category. Let $\ML$ be an action operad, and $P,Q$ be  $\ML$-operads in $C$ with a map $\sigma \colon P \to Q$ of $\ML$-operads between them. Then $\sigma$ induces a monad map $(\id, \sigma^*) \colon (C,\underline{Q}) \to (C,\underline{P})$ and hence a functor on categories of algebras.
\end{prop}
\begin{proof}
...
\[
g^{\otimes} \colon x_1 \otimes \cdots x_n \to x_{\pi(g)^{-1}(1)} \otimes \cdots x_{\pi(g)^{-1}(n)}
\]
...
\begin{proof}
Let $\ML$ be a crossed action operad, let $X$ be a $\ML$-monoidal category, and fix $x \in \mathrm{Ob}(X)$ and \( f\colon I \to I \). From \cref{surjortriv} we know that \( \pi \colon \Lambda(2) \to S_2 \) is surjective, so that the set $\pi^{-1}(  (1 \, 2)  )$ is non-empty,
...
\[
-\cdot \alpha \colon M(x_{1}, \ldots, x_{n}; y) \rightarrow M(x_{\pi(\alpha)(1)}, \ldots, x_{\pi(\alpha)(n)}; y);
\]
...
%\begin{longtable}{RLL}
%			f \otimes (\id_{w^*} \otimes f^{-1} \otimes \id_{v^*}) & = & \big( \, f \circ \id_w \, \big) \otimes \big( \, \id_{v^*} \circ  (\id_{w^*} \otimes f^{-1} \otimes \id_{v^*}) \, \big) \\
%			& = & \big( \, f \otimes \id_{v^*} \, \big) \circ \big( \, \id_w \otimes (\id_{w^*} \otimes f^{-1} \otimes \id_{v^*}) \, \big) \\
%			& = & ( f \otimes \id_{v^*} ) \circ ( f^{-1} \otimes \id_{v^*}) \\
%			& = & \id_I
%\end{longtable}
%and 
%\begin{longtable}{RLL}
%			(\id_{w^*} \otimes f^{-1} \otimes \id_{v^*}) \otimes f & = & \big( \, (\id_{w^*} \otimes f^{-1} \otimes \id_{v^*}) \circ \id_{w^*} \, \big) \otimes \big( \, \id_v \circ f \, \big) \\
%			& = & \big( \, (\id_{w^*} \otimes f^{-1} \otimes \id_{v^*}) \otimes \id_v \, \big) \circ \big( \, \id_{w^*} \otimes f \, \big) \\
...
\[ 
\begin{array}{rcl}
\mathrm{M}(L_{n})^{\mathrm{gp, ab}}  & \cong &  \bigquotient{{\mathrm{M}(\ELnn)}^{\mathrm{gp, ab}}}{Im\big( \, {\mathrm{M}(\tilde{\delta})}^{\mathrm{gp, ab}} - {\mathrm{M}(\tilde{I})}^{\mathrm{gp, ab}} \, \big)} \\

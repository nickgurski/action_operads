...
\begin{itemize}
\item $\Sigma_{n}$\nomenclature[N]{$\Sigma_{n}$}{symmetric group on $n$ letters} is the symmetric group on $n$ letters, and $Br_{n}$\nomenclature[N]{$Br_{n}$}{braid group on $n$ strands} is the braid group on $n$ strands.
...
\item Two less trivial examples are given by the braid groups, $\Lambda = \mb{Br}$\nomenclature[N]{$\mb{Br}$}{action operad of braid groups}, and the ribbon braid groups, $\Lambda = \mb{RBr}$\nomenclature[N]{$\mb{RBr}$}{action operad of ribbon braid groups}.  (A ribbon braid is given, geometrically, as a braid with strands replaced by ribbons in which we allow full twists.  The actual definition of the ribbon braid groups is as the fundamental group of a configuration space in which points have labels in the circle, $S^{1}$; see \cite{sal-wahl}.)  In each case, the homomorphism $\pi$ is given by taking underlying permutations, and the operad structure is given geometrically by using the procedure explained after Definition \ref{broperad}.  We refer the reader to \cite{fie-br} for more information about braided operads, and to \cite{sal-wahl, wahl-thesis} for information about the ribbon case.
...
\begin{Defi}
The category of algebras for $P$, $P\mbox{-}\mb{Alg}$\nomenclature[C]{$P\mbox{-}\mb{Alg}$}{of $P$-algebras and morphisms for an operad (or ($2$-)monad) $P$}, has objects the $P$-algebras $(X, \alpha)$ and morphisms $f: (X, \alpha) \rightarrow (Y, \beta)$ those functions $f:X \rightarrow Y$ such that the following diagram commutes for every $n$.
\[
\xy
...
\begin{eq*}\begin{array}{rll}
		 E\Lambda\mbox{-}\mathrm{Alg}_S( L\Big(E\Lambda(\underline{n}) \Big), X) & \cong  & E\Lambda\mbox{-}\mathrm{Alg}_S( E\Lambda(\underline{n}), X_{inv}) \\
...
By construction, $\tilde{\delta}$ and $\tilde{I}$ form a reflexive pair in $E\Lambda\mbox{-}\mathrm{Alg}_S$, so by \cref{refcoeq_calcs} we have the further corollary below.

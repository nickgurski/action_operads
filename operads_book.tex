...
This section will provide an introduction to the basic definitions of operads, including some specific types such as symmetric and braided operads.
 QQQ introduction to operads
 	\begin{itemize}
 		\item introduces notations and conventions
 		\item reviews definitions of operads (general basic overview) including symmetric, non-symmetric, and braided
 		\item remarks and examples are included
 	\end{itemize}
...
\end{thm}
\begin{proof}
First note that there is a category $\mb{Op}^{g}$\nomenclature[C]{$\mb{Op}^{g}$}{of operads $P$ for which each $P(n)$ has a group structure and operad maps}, whose objects are operads $P$ in which each $P(n)$ also carries a group structure. This is an equational theory using equations with only finitely many elements, so $\mb{Op}^{g}$ is locally finitely presentable. QQQ \cite[Corollary 3.7]{ar} QQQ? A slice category of a locally finitely presentable category is itself locally finite presentable \cite[Proposition 1.57]{ar} and since the symmetric operad is an object of $\mb{Op}^{g}$, the slice category\index{category!slice category} $\mb{Op}^{g}/\Sigma$ is locally finitely presentable.
...
\begin{prop} \label{invadj} The 2-functor $(\_)_{\mathrm{inv}}: \ELAlg \to \ELAlg$ has a left adjoint, $L: \ELAlg \to \ELAlg$.
\end{prop}

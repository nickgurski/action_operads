...
The functor $\pi_0 \circ L \circ \EL: \sets \to T_{\ML}\mbox{-}\textrm{Alg}$ is naturally isomorphic to the functor $\textrm{gp} \circ \pi_0 \circ \EL$, where $\textrm{gp}$ is the group completion functor.
\end{prop}
\begin{proof}
The proof is the same style as above, with both functors being  left adjoints of the functor giving the objects of $(DM)_{inv} = D(M^{\times})$.
\end{proof}
...
We summarize thes results below.
...
We now turn to our third adjunction, which is of a less formal nature than the first two.
...
\begin{Defi} Let $\mathrm{M} : \mathrm{MonCat} \to \mathrm{Mon}$ be the functor which sends monoidal categories $X$ to the quotient of their monoid of morphisms by the relation that sets $\otimes = \circ$.  

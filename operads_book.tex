...
\begin{center}
    \begin{tikzpicture}[x=20mm,y=15mm]
		...
		\node (b) at (1,0) {$\mathrm{Mor}(\ELnn)$};
		\node (c) at (0,-1) {$(s \times t)(\ELnn) $};
		\node (d) at (1,-1) {$\mathrm{Mor}(\ELnn)$};
...
\begin{eq*} L_n(I, I) \quad \cong \quad \bigquotient{{\mathrm{Mor}(L_n)}^{\mathrm{ab}}}{{(s \times t)(L_n)}^{\mathrm{ab}}} \end{eq*}
and therefore
\begin{eq*} \mathrm{Mor}(L_n) \quad \cong \quad (s \times t)(L_n) \, \times \, \bigquotient{{\mathrm{Mor}(L_n)}^{\mathrm{ab}}}{{(s \times t)(L_n)}^{\mathrm{ab}}}. \end{eq*}
\end{prop}
\begin{proof}
...
\end{prop}
\begin{proof}
Recall  that in any monoidal category with invertible objects,
\[ f' \circ f \quad = \quad f' \otimes \mathrm{id}_{y*} \otimes f \]
by \cref{tenscomp}, so both $\mathrm{Mor}(X)$ and $\mathrm{M}(X)$ are groups. Consider the canonical homomorphism $\psi: \mathrm{Mor}(X) \to \mathrm{M}(X) \to \mathrm{M}(X)^{\mathrm{ab}}$. For any composable pair $f: x \to y$, $f': y \to z$ in $X$, we have
...
\begin{eq*} \begin{array}{rll}
		...
		\end{array}
\end{eq*}
so $\psi(\mathrm{id}_{y*}) = e$ for any object $y$, so $\mathrm{Ob}(X)$ is contained in the kernel of $\psi$.
...
\begin{eq*} \mathrm{Mor}(L_n) \quad \cong \quad \mathbb{Z}^{\ast n} \times_{\mathbb{Z}^n} \mathbb{Z}^{\ast n}  \, \times \, \frac{\left(\quotient{{\mathrm{M}(\ELnn)}^{\mathrm{gp},\mathrm{ab}}}{\Delta}\right)}{\left(\quotient{{(\mathbb{Z}^{\ast n} \times_{\mathbb{Z}^n} \mathbb{Z}^{\ast n})}^{\mathrm{ab}}}{\mathbb{Z}^n}\right)} \end{eq*}
\end{prop}

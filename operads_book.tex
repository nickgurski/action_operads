...
This operad $RB$\index{action operad!of ribbon braid groups} is also clearly an action operad, since we can just define $\pi^{RB} \colon RB_{n} \to \Sigma_n$ to act like $\pi^B$ on any braids, at which point the fact that $\pi(t) \in S_1 = \{e_1\}$ will automatically take care of the twists.
...
\[
\textrm{Ker}\,\pi_n = \{g \in \Lambda(n)  \colon  \pi_{n}(g) = e_{n} \}
\]
...
An action operad $\mb{\Lambda}$ determines, and is determined by, the following \colon 
\begin{itemize}
...
Let $f \colon \ML \rightarrow \ML'$ be a map of action operads.  Then $f$ induces a functor $f^{*} \colon \ML'\mbox{-}\mb{Op} \to \ML\mbox{-}\mb{Op}$.
\end{prop}
...
\begin{enumerate}
\item There is an adjunction between the category of $\ML$-operads and the category of symmetric operads, with right adjoint $\pi^{*} \colon \ML\mbox{-}\mb{Op} \to \mb{\Sigma}\mbox{-}\mb{Op}$ and left adjoint denoted $S$.
...
\end{thm}
\begin{proof}
Given any map of monoids $f \colon M \to N$ in a monoidal category, there is an adjunction between right $M$-modules and right $N$-modules given by $f^{*}$ as the right adjoint and $A \mapsto A \otimes_{M} N$ as the left adjoint.  Thus we define \[
...
\begin{Defi}
Let $S$ be a monad on a category $C$ and $T$ be a monad on a category $D$. A \emph{monad map}\index{monad!maps} of from $S$ to $T$ is a functor $F \colon C \to D$ together with a natural transformation $\alpha \colonTF \Rightarrow FS$ such that the following diagrams commute.
...
Let $C$ be a cocomplete symmetric monoidal category. Let $\ML$ be an action operad, and $P,Q$ be  $\ML$-operads in $C$ with a map $\sigma \colon P \to Q$ of $\ML$-operads between them. Then $\sigma$ induces a monad map $(\id, \sigma^*) \colon (C,Q) \to (C,P)$ and hence a functor on categories of algebras.
\end{prop}

We can combine these two propositions.

\begin{cor}\label{monoidaladj_cor}
If $C, D, P, F$ are as in \cref{monoidal_to_monadmap}, and $F$ is part of a monoidal adjunction (i.e., an adjunction in which both functors are lax symmetric monoidal, and the unit and counit are monoidal transformations) $F \dashv U$, then $(F, \id)$ and $(U, \id)$ are both monad maps. The unit $\eta \colon1 \Rightarrow UF$ induces an operad map $\eta \colonP \Rightarrow UFP$, and a transformation between monad maps
\[
...
Note also that in the following definitions we will often write the composite
    ...
    \]
simply abbreviated as $\mu^P \times 1$.  Furthermore, instead of using an element $\textrm{id} \in P(1)$ as the operadic unit, we will now denote this as $\eta^{P} \colon 1 \rightarrow P(1)$.
\end{conv}
...
\begin{Defi}
Let $P$ be a $\ML$-operad. A \textit{pseudoalgebra}\index{algebra!pseudoalgebra}\index{operad!pseudoalgebra} for $P$ consists of \colon 
    \begin{itemize}
        \item a category $X$,
...
\begin{Defi}
Let $(X, \alpha_n,\phi,\phi_\eta)$ and $(Y, \beta_n,\psi,\psi_{\eta})$ be pseudoalgebras for a $\ML$-operad $P$. A \textit{pseudomorphism}\index{algebra!pseudomorphism} of $P$-pseudoalgebras consists of \colon 
    \begin{itemize}
...
\begin{Defi}
Let $(f, \overline{f}), (g, \overline{g}) \colon X \rightarrow Y$ be pseudomorphisms of $T$-algebras.  A \textit{$T$-transformation}\index{algebra!pseudomorphism!transformation} consists of a 2-cell $\gamma \colonf \Rightarrow g$ such that the following equality of pasting diagrams holds.
\[
        \xy
            (0,0)*+{TX}="00";
            (30,0)*+{TY}="10";
...
\end{prop}
\begin{proof}
To show that $\underline{P}$ is finitary we must show that it preserves filtered colimits \ref{def \colon filtered} or, equivalently, that it preserves directed colimits (see \cite{ar}). Consider some directed colimit, $\text{colim}X_{i}$ say, in $\mathbf{Cat}$. Then consider the following sequence of isomorphisms:
    \begin{align*}
...
Let $X,Y$ be strict $\EL$-algebras, and $F, G \colon X \to Y$ be strict $\EL$-algebra maps. The structure of an $\EL$-algebra 2-cell $\alpha \colonF \Rightarrow G$ determines and is determined by some stuff.
\end{prop}
\begin{proof}
...
\[
(!; \mathrm{id}, \ldots, \mathrm{id}) \colon  (e; x_1, \ldots, x_n) \to (g; x_1, \ldots, x_n)
\]
...
\endxy
\]
\item The composition product, along with the unit of the adjunction $\eta \colon1 \rightarrow T1$, give $T\mbox{-}\mb{Coll}$ a monoidal structure.  A \textit{$T$-operad}\index{monad!$T$-operad}\index{operad!$T$-operad} is a monoid in $T\mbox{-}\mb{Coll}$.
\end{enumerate}
\end{Defi}
...
\end{thm}
\begin{proof}
Let $K$ be such a club.  Our hypotheses immediately imply that $K$ is a groupoid with objects in bijection with the natural numbers; we will now assume the functor $K \to B\Sigma$ is the identity on objects.  Let $\Lambda(n) = K(n,n)$.  Now $K$ comes equipped with a functor to $B\Sigma$, in other words group homomorphisms $\pi_{n} \colon \Lambda(n) \rightarrow \Sigma_{n}$.  We claim that the club structure on $K$ makes the collection of groups $\{ \Lambda(n) \}$ an action operad.  In order to do so, we will employ \cref{thm:charAOp}.
...
\end{prop}
\begin{proof}
Consider the underlying monoidal category of $(C, \sigma)$, which we will just write as $C$.  We can find a strict monoidal category $C'$ by coherence for monoidal categories together with an equivalence, as monoidal categories, between $C$ and $C'$.  By standard methods, this can be improved to an adjoint equivalence between $C$ and $C'$ in the 2-category of monoidal categories, strong monoidal functors, and monoidal transformations.  Let $F \colon  C \rightarrow C', G \colon C' \rightarrow C$ be the functors in this adjoint equivalence, and $\eta \colon 1 \Rightarrow FG$ the unit (which we note for emphasis is invertible).  For objects $x,y \in C'$, we define a commutor $\sigma'$ for $C'$ as the composite
\[
\begin{array}{rcl}
...
\end{thm}
\begin{proof}
We will use \cref{thm:charAOp} to determine the rest of the action operad structure.  Thus we must give, for any collection of natural numbers $n, k_{1}, \ldots, k_{n}$ and $\underline{k} = \sum k_{i}$, group homomorphisms $\beta \colonJ_{k_{1}} \times \cdots \times J_{k_{n}} \rightarrow J_{\underline{k}}$ and functions $\delta \colon J_{n} \rightarrow J_{\underline{k}}$ satisfying nine axioms.    We define both of these on generators, starting with $\beta$.
...
\[ [ \, \_ \, ] \, \colon \, \mathrm{Ob}(\ELn) \to \pi_0(\ELn) \]
...
\[ \alpha( \, e_m \, ; \, \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m} \, ) \quad = \quad \mathrm{id}_{x_1} \otimes \ldots \otimes \mathrm{id}_{x_m} \quad = \quad \mathrm{id}_{x_1 \otimes \ldots \otimes x_m} \]
...
\begin{proof}
Let $\ML$ be a crossed action operad, let $X$ be a $E\Lambda$-algebra, and fix $x \in \mathrm{Ob}(X)$ and \( f\colon I \to I \). From QQQ Missing ref QQQ \cref{surjortriv} we know that \( \pi \colon G(2) \to S_2 \) is surjective, so that the set $\pi^{-1}( \, (1 \, 2) \, )$ is non-empty, and from the rules for composition of action morphisms we see that for any such $g \in \pi^{-1}( \, (1 \, 2) \, )$,
\[\begin{array}{rll}
...
\[ k \, \colon= \, \mu( \, h \ ; \, e_1, \, e_0 \, ) \]
and then consider
\[ g \, \colon= \, h \cdot \mu(e_2; k^{-1}, e_1) \] 
...
\[ \begin{array}{rlrll}
			...
			& \colon & (g;x_1, \ldots, x_m) & \mapsto & \alpha(g; \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m})
		\end{array}
\]
But by \cref{hom-set-lemma}, each element of $\mathrm{Mor}(\ELn)$ can be expressed in the form $\alpha(g; \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m})$ for a unique collection $(g;x_1, \ldots, x_m)$, and so this function $\alpha$ is actually a bijection of sets. Furthermore, this function preserves tensor product, since
\[ \begin{array}{rll}
...
Let $P$ be a $\ML$-operad. Then the following equip $\underline{P}$ with a pseudo-commutative structure.
    ...
    \end{itemize}
These must satisfy the following \colon 
    \begin{enumerate}
        \item For all $n \in \mathbb{N}_+$\nomenclature[N]{$\mathbb{N}_+$}{the set of positive natural numbers},
            \[
                t_{1,n} = e_n = t_{n,1}
            \]
             and for all $p \in P(n)$, the isomorphism $\lambda_{p, \textrm{id}}\colon p \cdot e_n \cong p$ is the identity map.
...
\right.
\]
For a functor $G \colon X \to Y$, it is easy to check that
\[
...
Given a bicategory $B$ and a pseudomonad $T$ on $B$, we can form the Kleisli bicategory of $T$, $\mb{Kl}_{T}$\nomenclature[C]{$\mb{Kl}_{T}$}{the Kleisli bicategory of a pseudomonad $T$}.  It has the same objects as $B$, but a 1-cell from $a$ to $b$ in  $\mb{Kl}_{T}$ is a 1-cell $f \colon a \rightarrow Tb$ in $B$.  In the case $B = \mb{Prof}, T = \widetilde{E\Lambda}$, the objects of $\mb{Kl}_{T}$ are categories, the 1-cells $X \srarrow Y$ are profunctors from $X$ to $E\Lambda Y$, or alternatively a functor $(E\Lambda Y)^{op} \times X \to \mb{Sets}$, and the 2-cells are natural transformation between such.
...
\item oplax monad maps $(F, \alpha)$ between them which are isomorphic to one of the form $(f^{+}, \alpha)$ for $f \colon S \to T$ for some function of the underlying sets, and
...
Let $x, y$ be invertible objects in a strict monoidal category $M$ with inverses $x^{-1}, y^{-1}$. Let $f \colon x \to y$, $g \colon x^{-1} \to y^{-1}$ be morphisms such that $f \otimes g = 1_I = g \otimes f$. Then $f, g$ are isomorphisms in $M$.
\end{lem}
\begin{proof}
...
A morphism $f \colon  w \to v$ in a strict monoidal category $X$ is \emph{invertible under tensor product} or \emph{has an inverse under tensor product} if there is another morphism $g \colon  w^{-1} \to v^{-1}$ such that $f \otimes g = \id_I = g \otimes f$; note that this requires $w, v$ to both be invertible with inverses $w^{-1}, v^{-1}$, respectively.
\end{Defi}

\begin{nota}\label{nota_tensinv}
For a morphism $f \colon w \to v$ in a strict monoidal category $X$, we write $f^*$ for its inverse under tensor product, if it exists. Standard arguments prove that $f^*$ is unique when it exists.
\end{nota}
...
\begin{itemize}
\item objects all functors $F \colon A \to X$ with the property that $Fa$ is invertible under tensor product or composition, respectively, in $X$ for all objects or morphisms, respectively, in $A$; and
...
\begin{Defi} Let $\mathrm{M} \colon \moncat \to \mon$ be the functor which sends a monoidal category $X$ to the quotient of its monoid of morphisms\index{monoidal category!monoid of morphisms} by the relation that sets $\otimes = \circ$. :
\[ \mathrm{M}X \quad = \quad \bigquotient{\mathrm{Mor}(X)}{f' \circ f \sim f' \otimes f}.\]
If $F \colon X \to Y$ is a strict monoidal functor, then $MF$ is the monoid homomorphism sending the class of a morphism $g$ to the class of $Fg$; the reader will easily check this is well-defined. We will call $\mathrm{M}X$ the \emph{collapsed} morphisms of $X$.
...
\begin{prop}\label{Moradj} $\mathrm{B} \colon  \cmon \to \moncat$ is a right adjoint to the functor $\mathrm{M}(\, - \,)^{\mathrm{ab}} \colon \moncat \to \cmon$.
\end{prop}
\begin{proof}
...
\begin{Defi}
We will say that a functor $F \colon  C \to D$ is \emph{surjective}\index{functor!surjective} if the underlying map of graphs is surjective. In particular $F$ must be surjective on objects, but it need not be full.
\end{Defi}
...
\begin{prop} \label{stGnsub} $\mathrm{Mor}(\ELn) \to (s \times t)(\ELn)$ is a split epi of monoids, so $(s \times t)(\ELn)$ is (isomorphic to) a submonoid of $\mathrm{Mor}(\ELn)$. Furthermore, we can choose the injection $i \colon (s \times t)(\ELn) \to \mathrm{Mor}(\ELn)$ such that the following diagram commutes.
\begin{center}
...
\[ \begin{array}{rlrll}
			...
			& \colon & \mathbb{N}^{\ast n} & \to & \lop \times_{\mathbb{N}} \mathbb{N}^{\ast n} \\
...
\begin{prop} \label{stZsub} $\MorLn \to (s \times t)(L_n)$ is a split epi of monoids, so $(s \times t)(L_n)$ is (isomorphic to) a submonoid of $\MorLn$. Furthermore, we can choose the injection $i \colon (s \times t)(L_n) \to \MorLn$ such that the following diagram commutes.
\begin{center}
...
\end{prop}
\begin{proof}
Let $i \colon (s \times t)(\ELnn) \to \mathrm{Mor}(\ELnn)$ be a splitting as in \cref{stGnsub}. We will first show that the function $\rho$ can be chosen to make the square
\begin{center}
    \begin{tikzpicture}[x=20mm,y=15mm]
...
The functor $q \colon \ELnn \to L_n$ is the cokernel of $\delta$, so the composite
\[
...
\]
exhibits $(s \times t)(L_n)$ as the cokernel of $(s \times t)(\delta)$ QQQ this is \cref{monoid_coeq} QQQ so induces a unique monoid homomorphism $i \colon  (s \times t)(L_n) \to \MorLn$ making the square below commute.
\begin{center}
    \begin{tikzpicture}[x=20mm,y=15mm]
		\node (a) at (0,0) {$(s \times t)(\ELnn)$};
		\node (b) at (2,0) {$\mathrm{Mor}(\ELnn)$};
...
\begin{cor} \label{tensinv} Every morphism $f \colon  w \to v$ in $L_n$ has an inverse under tensor product, $f^* \colon  w^* \to v^*$. That is, the monoid $\MorLn$ is actually a group and $L_n(I,I)$ is a subgroup.
\end{cor}
...
Recall that the functor $q \colon  \ELnn \to L_n$ expresses $L_n$ as a reflexive coequalizer of $\ML$-monoidal categories and hence also as a reflexive coequalizer of mere monoidal categories by \cref{q_other_coeq2}. We can therefore use the adjunction from \cref{Moradj} to calculate $\MLn^{\mathrm{gp},\mathrm{ab}}$. 
...
\[ \begin{array}{c}
			\big[ \, \alpha^{\ELnn}\big( \, \mu( \, g \, ; \, e_{|\tilde{\delta}(x_1)|}, \ldots, e_{|\tilde{\delta}(x_m)|} \, ) \, ; \, \mathrm{id}_{x'_1}, \ldots,  \mathrm{id}_{x'_{m'}} \, \big) \, \big] \\
			\, \otimes \, \\
			\big[ \, \alpha^{\ELnn}\big( \, \mu( \, g \, ; \, e_{|\tilde{I}(x_1)|}, \ldots, e_{|\tilde{I}(x_m)|} \, ) \, ; \, \mathrm{id}_{x''_1}, \ldots,  \mathrm{id}_{x''_{m''}} \, \big) \, \big]^*
		\end{array}
\] 
...
By \cref{hom-set-lemma}, we can express this isomorphism as $h^{\otimes}$ for some $h \in \Lambda(p)$we know it must be possible to express the action morphism $\alpha^{\ELnn}(g; \mathrm{id}_{\tilde{\delta}(x_1)}, \ldots, \mathrm{id}_{\tilde{\delta}(x_m)})$ as an action morphism on the identities of generators. Since the source of this map is
...
\[ [ \, \alpha^{\ELnn}(g; \mathrm{id}_{\tilde{\delta}(x_1)}, \ldots, \mathrm{id}_{\tilde{\delta}(x_m)})  \, ] \quad = \quad \big[ \, \alpha^{\ELnn}\big( \, \mu( \, g \, ; \, e_{|\tilde{\delta}(x_1)|}, \ldots, e_{|\tilde{\delta}(x_m)|} \, ) \, ; \, \mathrm{id}_{x'_1}, \ldots, \mathrm{id}_{x'_{m'}} \, \big) \, \big] \]
For analogous reasons we also get
\[ \begin{array}{rll}
			\mathrm{M}(\tilde{I})^{\mathrm{gp, ab}}(f) & = & [ \, \alpha^{\ELnn}(g; \mathrm{id}_{\tilde{I}(x_1)}, \ldots, \mathrm{id}_{\tilde{I}(x_m)}) \, ]  \\
			& = &  \big[ \, \alpha^{\ELnn}\big( \, \mu( \, g \, ; \, e_{|\tilde{I}(x_1)|}, \ldots, e_{|\tilde{I}(x_m)|} \, ) \, ; \, \mathrm{id}_{x''_1}, \ldots,  \mathrm{id}_{x''_{m''}} \, \big) \, \big]
		\end{array}
\]
...
\end{lem}
\begin{proof}
Let  $f \colon  x \to y$, $f' \colon  y \to z$ be a composable pair of morphisms in $X$. Recall  that in any monoidal category with invertible objects,
\[ f' \circ f \quad = \quad f' \otimes \mathrm{id}_{y^{-1}} \otimes f \]
by \cref{tenscomp}, so both $\mathrm{Mor}(X)$ and $\mathrm{M}(X)$ are groups. Consider the canonical homomorphism $\psi \colon  \mathrm{Mor}(X) \to \mathrm{M}(X) \to \mathrm{M}(X)^{\mathrm{ab}}$. Then we have
\[ \begin{array}{rll}
...
Now let $A$ be an abelian group and $\phi \colon  \mathrm{Mor}(X)^{\mathrm{ab}} \to A$ any homomorphism of groups which satisfies the condition $\phi(\mathrm{ab}(\mathrm{id}_x)) = e$ for all objects $x$. Then
\[ \begin{array}{rll}
...
\[ \begin{array}{rcrcl}
			\psi & \colon & \mathbb{S}_{2n} & \to & Z \\
			 & \colon & z_i & \mapsto & z_i \\
			 & \colon & z_{n+i} & \mapsto & z_i^* \\
			 & \colon & \alpha(\sigma; \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m}) & \mapsto & (x_1 \otimes \ldots \otimes x_m, x_{\sigma(1)} \otimes \ldots \otimes x_{\sigma(m)})
		\end{array}
\]
...
\[ \begin{array}{rlll}
			\alpha( \, \sigma \, ; \, \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m} \, ) & = & \alpha( \, \sigma_{i_k} \cdot \ldots \cdot \sigma_{i_1} \, ; \,  \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m} \, ) \\
			& = &  \alpha( \, \sigma_{i_1} \, ; \,  \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m} \, )  \\
			& & \circ \, \alpha( \, \sigma_{i_2} \, ; \,  \mathrm{id}_{x_{\sigma_{i_1}(1)}}, \ldots, \mathrm{id}_{x_{\sigma_{i_1}(m)}} \, ) \, \circ \ldots \\
			& & \circ \, \alpha( \, \sigma_{i_k} \, ; \,  \mathrm{id}_{x_{\sigma_{i_{k-1}} \cdot \ldots \cdot \sigma_{i_1}(1)}}, \ldots, \mathrm{id}_{x_{\sigma_{i_{k-1}} \cdot \ldots \cdot \sigma_{i_1}(m)}} \, )
		\end{array}
\]
Then if $\sigma_i = (i \, \, i+1) \in \Sigma_m$ is some elementary transposition we will have
\[ \begin{array}{rll}
			\alpha( \, (i \, \, i+1) \, ; \, \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m} \, ) & = & \alpha( \, e_{i-1} \otimes (1 2) \otimes e_{m-i-1} \, ; \,  \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m} \, ) \\
			& = & \mathrm{id}_{x_1 \otimes \ldots \otimes x_{i-1}} \otimes \alpha( \, (1 2) \, ; \, \mathrm{id}_{x_i}, \mathrm{id}_{x_{i+1}} \, ) \otimes \mathrm{id}_{x_{i+2} \otimes \ldots \otimes x_m}
		\end{array}
\]
Therefore all of the morphisms of $\mathbb{S}_{2n}$ are generated by just the identities and the action maps $\alpha( \, (1 2) \, ; \, \mathrm{id}_{x_1}, \mathrm{id}_{x_2} \, )$ for all pairs $x_1, x_2 \in \{z_1, \ldots, z_{2n} \}$. Passing through $\psi$, this means that elements of $\mathbb{Z}^{\ast n} \times_{\mathbb{Z}^n} \mathbb{Z}^{\ast n}$ can always be expressed as a tensor product of elements of the form
...
and without loss of generality assume that $|g| \ge |g'|$, so also $|h| \le |h'|$. The operadic product $\mu(h; e_0, \ldots, e_0)$ is clearly an element of the group $\LL(0)$, and we know from \cref{G0abel} that this is an abelian group under tensor product, so also let $\mu(h; e_0, \ldots, e_0)^*$ be its inverse. Then
\[ \begin{array}{rll}
...
\begin{cor} \label{gpcompin} The canonical map $\mathrm{gp} \colon (\ML, \otimes) \to (\ML, \otimes)^{\mathrm{gp}}$ associated with the group completion of $\ML$ is an inclusion. Further, the canonical map $\mathrm{gp} \colon (\ML, \otimes) \times_{\mathbb{N}} \mathbb{N}^{\ast n} \to ((\ML, \otimes) \times_{\mathbb{N}} \mathbb{N}^{\ast n})^{\mathrm{gp}}$  is also an inclusion.
\end{cor}
\begin{proof}
...
\[ \ML \times_{\mathbb{N}} \mathbb{N}^{\ast n} \quad = \quad \mathbb{N}^{\ast ( \, |\mathcal{G}_0| + n|\mathcal{G}_1| + n^2 |\mathcal{G}_2| + \ldots \, )} \]
\end{prop}
\begin{proof}
Let $(g, w)$ be an arbitrary element of $\ML \times_{\mathbb{N}} \mathbb{N}^{\ast n}$. The monoid $\ML$ is free of the generators $\mathcal{G}$, and $\mathbb{N}^{\ast n}$ is free on $\{z_1, \ldots, z_n\}$, so we can find unique expansions of $g$ and $w$ as tensor products
\[ \begin{array}{rclcrcl}
			g & = & g_1 \otimes \ldots \otimes g_k, & \quad & g_1, \ldots, g_k & \in & \mathcal{G} \\
			w & = & x_1 \otimes \ldots \otimes x_m, & \quad & x_1, \ldots, x_m & \in & \{z_1, \ldots, z_n\}
		\end{array}
\]
But each of the generators $z_1, \ldots, z_n$ has length 1, so the index $m$ here is really just the length $|w|$, which by the definition of $\ML \times_{\mathbb{N}} \mathbb{N}^{\ast n}$ is also the length $|g| = |g_1| + \ldots + |g_k|$. Therefore we may write
\[ \begin{array}{rll}
			(g, w) & = & (g_1 \otimes \ldots \otimes g_k, x_1 \otimes \ldots \otimes x_{|w|}) \\
			& = & (g_1, x_1 \otimes \ldots \otimes x_{|g_1|}) \otimes (g_2, x_{|g_1|+1} \otimes \ldots \otimes x_{|g_1|+|g_2|}) \otimes \ldots \\
			& & \otimes (g_k, x_{|g_1| + \ldots + |g_{k-1}|+1} \otimes \ldots \otimes x_{|g_1| + \ldots + |g_k|})
		\end{array}
\]
...
\[ \begin{array}{rl}
			& (g_1, x_1 \otimes \ldots x_{|g_1|}) \otimes \ldots \otimes (g_k, x_{|g_1| + \ldots + |g_{k-1}|+1} \otimes \ldots \otimes x_{|g_1| + \ldots + |g_k|}) \\
			= & (g'_1, x'_1 \otimes \ldots \otimes x'_{|g'_1|}) \otimes \ldots \otimes (g'_{k'}, x'_{|g'_1| + \ldots + |g'_{k'-1}|+1} \otimes \ldots \otimes x'_{|g'_1| + \ldots + |g'_{k'}|})
		\end{array}
\]
\[ \begin{array}{rcclcccl}
			\implies \quad \quad & g_1 \otimes \ldots \otimes g_k & = & g'_1 \otimes \ldots \otimes g'_{k'}, & \quad \quad & x_1 \otimes \ldots \otimes x_{m} & = & x'_1 \otimes \ldots \otimes x'_{m'} \\
			& & & & & & & \\
...
\[ \begin{array}{rll}
			|\mathcal{G} \times_{\mathbb{N}} \mathbb{N}^{\ast n}| & = & |\mathcal{G}_0| \cdot |\{ z_1, \ldots, z_n \}^0|  \, + \, |\mathcal{G}_1| \cdot |\{ z_1, \ldots, z_n \}^1| \, + \, |\mathcal{G}_2| \cdot |\{ z_1, \ldots, z_n \}^2| \, + \, \ldots \\
			& = & |\mathcal{G}_0| + n|\mathcal{G}_1| + n^2 |\mathcal{G}_2| + \ldots 
		\end{array}	
\]
\end{proof}
...
\[ (\ML \times_{\mathbb{N}} \mathbb{N}^{\ast n})^{\mathrm{gp}, \mathrm{ab}} \quad = \quad \mathbb{Z}^{|\mathcal{G}_0| + n|\mathcal{G}_1| + n^2 |\mathcal{G}_2| + \ldots}. \]
\end{cor}
...
\[ \bigquotient{\mathbb{Z}^{|\mathcal{G}_0| + n|\mathcal{G}_1| + n^2 |\mathcal{G}_2| + \ldots}}{\otimes \sim \circ} \quad \quad \cong \quad \quad \bigquotient{\mathbb{Z}^{|\mathcal{G}'_0| + n|\mathcal{G}'_1| + n^2 |\mathcal{G}'_2| + \ldots}}{\otimes \sim \circ} \]
\end{cor}
\begin{proof}
...
\end{enumerate}
Also, we will use $\Psi \colon  G \times_{\mathbb{N}} \mathbb{N}^{\ast 2n} \to A(G,n)$ to represent the corresponding quotient map. 
\end{Defi}
...
\[ \begin{array}{c}
			...
			= \\
			\big( \, \, \bigotimes_i x_i, \quad \bigotimes_i y_{\pi(g^{-1})(i)}, \quad \Psi \alpha_{\ELnn}( \, g \, ; \, \mathrm{id}_{q^{-1}(y_1)}, \ldots, \mathrm{id}_{q^{-1}(y_m)} \, \, ) \, \otimes \, (\bigotimes_i h_i) \, \big) 
		\end{array}
\]
\end{thm}
\begin{proof}
...
\[ \alpha(g; f_1, \ldots, f_m) \quad = \quad \alpha(g; \mathrm{id}_{y_1}, \ldots, \mathrm{id}_{y_m}) \circ (f_1 \otimes \ldots \otimes f_m) \]
for all morphisms $f_i \colon  x_i \to y_i$, and
\[ \begin{array}{rl}
			& \alpha( \, g \, ; \, \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_{i-1}}, \mathrm{id}_{x_i \otimes x'_{i}}, \mathrm{id}_{x_{i+1}}, \ldots \mathrm{id}_{x_m} \, ) \\
			= & \alpha\big( \, g \, ; \, \alpha(e_1;\mathrm{id}_{x_1}), \ldots, \alpha(e_1;\mathrm{id}_{x_{i-1}}), \alpha(e_2;\mathrm{id}_{x_i}, \mathrm{id}_{x'_i}), \alpha(e_1;\mathrm{id}_{x_{i+1}}), \ldots,  \alpha(e_1;\mathrm{id}_{x_m}) \, \big) \\
			= & \alpha\big( \, \mu(g; e_1, \ldots, e_1, e_2, e_1, \ldots, e_1) \, ; \, \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_{i-1}}, \mathrm{id}_{x_i}, \mathrm{id}_{x'_{i}}, \mathrm{id}_{x_{i+1}}, \ldots, \mathrm{id}_{x_m} \, \big)
		\end{array}
\]
...
\end{tikzcd} \]
When $i \neq j$, the equivalence classes of the morphisms $\alpha((1 \, 2);\mathrm{id}_{z_i}, \mathrm{id}_{z_j})$ get sent to zero by the rightmost arrow, whereas the $\alpha((1 \, 2);\mathrm{id}_{z_i}, \mathrm{id}_{z_i})$ are each sent to a different generator of $\mathbb{Z}_2^{n}$, which is denoted by the appropriate $n$-tuple $(0,\ldots,0, 1, 0,\ldots,0)$. 
...
\[ \alpha( \, t \, ; \, \mathrm{id}_{z_i} \, ) \quad = \quad \big( \, z_i, \, z_i, \, (0,\ldots,0, 1, 0,\ldots,0) \, \big) \]
...
\[ \alpha_{L_n}( \, g \, ; \, \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m} \, ) \quad = \quad \mathrm{id}_{x_1 \otimes \ldots \otimes x_m} \]
\end{lem}
\begin{proof}
In order for $\alpha_{L_n}$ to be a well-defined $E\Lambda$-action, the map $\alpha_{L_n}(g; \mathrm{id}_{x_1}, \ldots, \mathrm{id}_{x_m})$ needs to have source $x_1 \otimes \ldots \otimes x_m$ and target $x_{\pi(g^{-1})(1)} \otimes \ldots \otimes x_{\pi(g^{-1})(m)}$, where by non-crossedness of $G$ the latter is also $x_1 \otimes \ldots \otimes x_m$. But we know from \cref{trivendo} that all morphisms in this $L_n$ are identities, and hence we get the result.
\end{proof}
...
\[ \begin{array}{rl} 
			& \alpha_{L_n}\big( \, g \, ; \, (x_1, y_1, h_1), \ldots, (x_m, y_m, h_m) \, \big) \\
			= & \alpha_{L_n}( \, g \, ; \, \mathrm{id}_{y_1}, \ldots, \mathrm{id}_{y_m} \, ) \circ \big( \, (x_1, y_1, h_1) \otimes \ldots \otimes (x_m, y_m, h_m) \, \big) \\
			= & \alpha_{L_n}( \, g \, ; \, \mathrm{id}_{y_1}, \ldots, \mathrm{id}_{y_m} \, ) \circ ( \, x_1 \otimes \ldots \otimes x_m, \, y_1 \otimes \ldots \otimes y_m, \, h_1 \otimes \ldots \otimes h_m \, ) \\
			= & \alpha_{L_n}( \, g \, ; \, \mathrm{id}_{y_1}, \ldots, \mathrm{id}_{y_m} \, ) \otimes \mathrm{id}_{y_1 \otimes \ldots \otimes y_m}^* \otimes ( \, x_1 \otimes \ldots \otimes x_m, \, y_1 \otimes \ldots \otimes y_m, \, h_1 \otimes \ldots \otimes h_m \, ) \\
		\end{array}
\]
Since we already understand tensor products of objects and unit endomorphisms, we now only need to find the action morphisms on identities. Moreover, we know that the source and target of $\alpha_{L_n}(g; \mathrm{id}_{y_1}, \ldots, \mathrm{id}_{y_m})$ have to be $y_1 \otimes \ldots \otimes y_m$ and $y_{\pi(g^{-1})(1)} \otimes \ldots \otimes y_{\pi(g^{-1})(m)}$ respectively, so to see this morphism as an element of the monoid
\[ \MorLn \quad \cong \quad (s \times t)(L_n) \times L_n(I,I) \]
...
In particular, for the identities $\mathrm{id}_{y_i} \in \mathrm{Mor}(L\mathbb{G}_{n})$ we can choose $\mathrm{id}_{q^{-1}(y_i)} \in \mathrm{Mor}(\ELnn)$, as by design $q(\mathrm{id}_{q^{-1}(y_i)}) = \mathrm{id}_{qq^{-1}(y_i)} = \mathrm{id}_{y_i}$. This means that if we denote by $p_I \colon  \mathrm{Mor}(L\mathbb{G}_{n}) \to L\mathbb{G}_{n}(I,I)$ the projection onto unit endomorphisms, we will have
\[ p_I \big( \, \alpha_{L_n}( \, g \, ; \, \mathrm{id}_{y_1}, \ldots, \mathrm{id}_{y_m} \, ) \, \big) \quad = \quad  p_I q\big( \, \alpha_{\ELnn}( \, g \, ; \, \mathrm{id}_{q^{-1}(y_1)}, \ldots, \mathrm{id}_{q^{-1}(y_m)} \, ) \, \big) \]
...
\[ \begin{array}{rll}
			p_I \big( \, \alpha_{L_n}( \, g \, ; \, \mathrm{id}_{y_1}, \ldots, \mathrm{id}_{y_m} \, ) \, \big) & = &  p_I q\big( \, \alpha_{\ELnn}( \, g \, ; \, \mathrm{id}_{q^{-1}(y_1)}, \ldots, \mathrm{id}_{q^{-1}(y_m)} \, ) \, \big) \\
			& = & \Psi \big( \, \alpha_{\ELnn}( \, g \, ; \, \mathrm{id}_{q^{-1}(y_1)}, \ldots, \mathrm{id}_{q^{-1}(y_m)} \, ) \, \big)
		\end{array}
\]
...
\[ \begin{array}{c}
			\alpha_{L_n}\big( \, g \, ; \, (x_1, y_1, h_1), \ldots, (x_m, y_m, h_m) \, \big) \\
			= \\
			\alpha_{L_n}( \, g \, ; \, \mathrm{id}_{y_1}, \ldots, \mathrm{id}_{y_m} \, ) \circ \, \bigotimes_i (x_i, y_i, h_i) \\
			= \\
			\big( \, \bigotimes_i y_i, \, \bigotimes_i y_{\pi(g^{-1})(i)}, \, \Psi \alpha_{\ELnn}( \, g \, ; \, \mathrm{id}_{q^{-1}(y_1)}, \ldots, \mathrm{id}_{q^{-1}(y_m)} \, ) \, \big) \otimes \, \mathrm{id}_{\otimes_i y_i}^* \otimes ( \, \bigotimes_i x_i, \, \bigotimes_i y_i, \, \bigotimes_i h_i \, ) \\
			= \\
			\big( \, \, \bigotimes_i x_i, \quad \bigotimes_i y_{\pi(g^{-1})(i)}, \quad \Psi \alpha_{\ELnn}( \, g \, ; \, \mathrm{id}_{q^{-1}(y_1)}, \ldots, \mathrm{id}_{q^{-1}(y_m)} \, \, ) \, \otimes \, (\bigotimes_i h_i) \, \big) 
		\end{array}
\]

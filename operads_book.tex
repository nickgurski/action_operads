...
\end{prop}
\begin{proof}
Let $i:(s \times t)(\ELn) \to \mathrm{Mor}(\ELn)$ be a splitting as in \cref{stGnsub}. We will first show that the function $\rho$ can be chosen to make the square
\begin{center}
    \begin{tikzpicture}[x=20mm,y=15mm]
		\node (a) at (0,0) {$(s \times t)(\ELn)$};
		\node (b) at (1,0) {$\mathrm{Mor}(\ELn)$};
		\node (c) at (0,-1) {$(s \times t)(\ELn) $};
...
Now the functor $\mathrm{Mor}$ is a left adjoint, so 
\[
\mathrm{Mor}(\ELn) \stackrel{\mathrm{Mor}q}{\longrightarrow} \mathrm{Mor}(\ELn) \longrightarrow \mathrm{Mor}(L_n)
\]
is a cokernel diagram in monoids.

Claim: $s \times t$ version also cokernel, get unique induced map.

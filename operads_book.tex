...
Finally, \cref{Gnmapsaction} also gives a complete description of how the morphisms of $\ELn$ interact as a monoid under tensor product, though to best express this we need some new terminology.

\begin{Defi} Let $\ML$ be an action operad. Then we will also use the notation $\Lambda$ to denote the \emph{underlying monoid}\index{action operad!underlying monoid} of this action operad. The underlying set of $\ML$ is the disjoint union, $\coprod_n \Lambda(n)$, with tensor product as the binary operation, $g \otimes h = \mu(e_2; g, h)$.
...
\[
y = xx^{-1} y \stackrel{\mathrm{id}_x g \mathrm{id}_y}{\longrightarrow} xy^{-1}y = x
\]
by a similar proof to that above.
\end{proof}
...
\end{lem}
\begin{proof}
A map $h: B \to X$ coequalizes $f;\id_B$ and $g; \id_B$ if and only if $hf = hg$ and $h \id_B = h \id_B$ by the universal property of the coproduct, so $c$ is still the universal map which coequalizes. Reflexivity is easy, as the map $B \coprod B \to A \coprod B$ which is $i;i$ for $i:B \to A \coprod B$ the canonical inclusion is a common section.
...
Since the underlying permutation maps $\pi : \Lambda(m) \to \Sigma_m$ of a crossed action operad $G$ are all surjective, we can always find an element of $g \in G(m)$ for which $\pi(g) = \sigma$. 
...
\begin{conv}
In what follows QQQ forward ref all the places we need this?, we fix an isomorphism $\MorLn \cong (s \times t)(L_n) \times L_n(I,I)$; we note that this isomorphism (and in particular the projection homomorphism $\MorLn \to L_n(I,I)$) depends on the choices made in the proof of \cref{stZsub}.
...
\begin{enumerate}
\item Determine the group $L_n(I,I)$ in such a way as to make the equality of two elements decidable.
...
This is a curious result. The morphisms $I \to I$ in $\ELn$ are exactly the elements of the group $\Lambda(0)$, but all of these become the identity in $L_n$. In other words, $L_n$ cannot detect any morphisms in $\Lambda(0)$, an idea we make precise now. 

...
\item $0 < \pi(k) < \pi(g|)$ QQQ Vertical line? QQQ: In this case $\pi(g) > \pi(g')$ and thus $g \neq g'$. Therefore we could change our presentation of $M$ by replacing $g$ with $k$ in the generator set $\mathcal{G}$, and also $h \otimes g = h' \otimes g'$ by $h' = h \otimes k$ in the relations $\mathcal{R}$. QQQ Does $h \otimes g$ mean the same thing as $hg$? QQQ
\end{itemize}

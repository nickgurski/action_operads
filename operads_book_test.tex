...
\]
If we write $p_I::  \mathrm{Mor}(L_{n}) \to L_{n}(I,I)$ the projection onto the automorphisms of the unit, we will therefore have
...
\mathrm{M}(\ELnn)^{\mathrm{gp},\mathrm{ab}} \ar[rr, "\mathrm{M}(q)^{\mathrm{gp},\mathrm{ab}}"] & & \mathrm{M}(L_n)^{\mathrm{gp},\mathrm{ab}} \ar[rr] & & \bigquotient{\mathrm{M}(L_{n})^{\mathrm{gp}, \mathrm{ab}}}{\mathbb{Z}^{{n}\choose{2}}}
\end{tikzcd} \]
...
\path[->,auto,mm] %arrows;  have to use this instead of tikzar, 
                    % for some reason
  (a) edge node {} (b)
  (b) edge node {s \times t} (c)
  (c) edge[swap, bend right=25] node {i} (b)
\end{tikzpicture}
\end{center}
...
Thus all of the regions within the diagram commute, and hence so will the outside. That is, $p_I \circ q$ is equal to the composite along the left, bottom, and then right edges, which by QQQ refs is $\Psi$. 

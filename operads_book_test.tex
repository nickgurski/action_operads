...
Repeat some adjoint functor theorem arguments
...
\begin{conv}
In what follows, we fix an isomorphism $\mathrm{Mor}(L_n) \cong (s \times t)(L_n) \times L_n(I,I)$; we note that this isomorphism (and in particular the projection homomorphism $\mathrm{Mor}(L_n) \to L_n(I,I)$) depends on the choices made in the proof of \cref{stZsub}.
...
\item $f = g$,
\item $g^{-1} f = \id_x$, and
\item the image of $g^{-1} f$ under the homomorphism $\mathrm{Mor}(L_n) \to (s \times t)(L_n)$ lands in the subgroup $\mathrm{Ob}(L_n) \subseteq (s \times t)(L_n)$. QQQ this seems wrong to me, does it just compute if s/t are the same?
\end{enumerate}
\end{thm}
\begin{proof}
QQQ insert
...
QQQ Can we improve this slightly? What do we know about $\mathrm{Ob}(L_n) \subseteq (s \times t)(L_n)$?

QQQ $L_n(I,I) \to L_n(x,x)$ iso by invertibility, sends identity to identity

\chapter{Computing automorphisms of the unit}

\section{Used to be somewhere else: now on to nasty quotients}


 It will, however, furnish us with all the group actions required to give $L_n$ a $\ML$-monoidal structure once we have given it a mere \emph{monoidal} one using that $q$ is surjective.  QQQ Forward ref to where we accomplish this.
